...
lib_A734::exo_repulsor_think()
{
	...
	lib_A734::exorepulsorinit();
	thread lib_A734::monitorplayerdeath();
	thread lib_A734::wait_for_game_end();
	for(;;)
	{
		self waittill("exo_ability_activate",var_00);
		if(var_00 == "exorepulsor_equipment_mp")
		{
			thread lib_A734::tryuserepulsor();
		}
	}
}
...
//Function Number: 4
lib_A734::tryuserepulsor(param_00)
{
	self endon("exo_repulsor_taken");
	if(self.repulsoractive == 1)
	{
		thread lib_A734::stop_repulsor(1);
		return;
	}

	thread lib_A734::start_repulsor();
}

//Function Number: 5
lib_A734::killrepulsorfx()
{
...
lib_A734::start_repulsor()
{
	...
	thread lib_A734::do_exo_repulsor();
...
//Function Number: 7
lib_A734::stop_repulsor(param_00)
{
...
//Function Number: 8
lib_A734::monitorplayerdeath()
{
	level endon("game_ended");
	self endon("disconnect");
	common_scripts\utility::waittill_any("death","joined_team","faux_spawn","exo_repulsor_taken");
	self.var_7011 = 0;
	thread lib_A734::stop_repulsor(0);
}

//Function Number: 9
lib_A734::update_exo_battery_hud()
{
...
//Function Number: 12
lib_A734::give_exo_repulsor()
{
	self giveweapon("exorepulsor_equipment_mp");
	thread lib_A734::exo_repulsor_think();
}

//Function Number: 13
lib_A734::wait_for_game_end()
{
	self endon("death");
	self endon("disconnect");
	self endon("joined_team");
