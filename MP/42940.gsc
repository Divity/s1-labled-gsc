...
lib_A7BC::init()
{
	...
	level.killstreakfuncs["guard_dog"] = ::lib_A7BC::tryusedog;
}

//Function Number: 2
lib_A7BC::setup_callbacks()
{
	level.agent_funcs["dog"] = level.agent_funcs["player"];
	level.agent_funcs["dog"]["spawn"] = ::lib_A7BC::spawn_dog;
	level.agent_funcs["dog"]["on_killed"] = ::lib_A7BC::on_agent_dog_killed;
	level.agent_funcs["dog"]["on_damaged"] = ::maps\mp\agents\_agents::on_agent_generic_damaged;
	level.agent_funcs["dog"]["on_damaged_finished"] = ::lib_A7BC::on_damaged_finished;
	level.agent_funcs["dog"]["think"] = ::maps\mp\agents\dog\_dog_think::main;
	level.killstreakwieldweapons["agent_mp"] = "agent_mp";
}

//Function Number: 3
lib_A7BC::tryusedog(param_00,param_01)
{
	return lib_A7BC::usedog();
}
...
lib_A7BC::usedog()
{
	...
	var_02 = maps\mp\agents\_agent_common::connectnewagent("dog",self.inliveplayerkillstreak);
	if(!isdefined(var_02))
	{
		return 0;
	}

	var_02 maps\mp\agents\_agent_utility::set_agent_team(self.inliveplayerkillstreak,self);
	var_03 = var_01.var_2E6;
	var_04 = vectortoangles(self.var_2E6 - var_01.var_2E6);
...
//Function Number: 5
lib_A7BC::on_agent_dog_killed(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08)
{
	self.isactive = 0;
	self.hasdied = 0;
	...
	var_0A = function_0063(var_09);
	param_08 = int(var_0A * 1000);
	self.body = self cloneagent(param_08);
	self playsoundasmaster("anml_doberman_death");
	maps\mp\agents\_agent_utility::deactivateagent();
	self notify("killanimscript");
}

//Function Number: 6
lib_A7BC::on_damaged_finished(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09)
{
	if(!isdefined(self.playing_pain_sound))
	{
		thread lib_A7BC::play_pain_sound(2.5);
	}
	...
	if(isplayer(param_01))
	{
		...
		{
			if(distancesquared(self.var_2E6,param_01.var_2E6) <= self.dogdamagedradiussq)
			{
...
//Function Number: 7
lib_A7BC::play_pain_sound(param_00)
{
	self endon("death");
	self playsoundasmaster("anml_doberman_pain");
...
//Function Number: 8
lib_A7BC::spawn_dog(param_00,param_01,param_02)
{
	if(function_0282())
	{
	...
	else
	{
		...
		var_03 = var_05.var_2E6;
		var_04 = var_05.var_41;
	}
	...
	{
		maps\mp\agents\_agent_utility::set_agent_team(param_02.inliveplayerkillstreak,param_02);
	}

	self sentient_setthreatbiasgroup("Dogs");
	self takeallweapons();
