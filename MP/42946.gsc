...
lib_A7C2::init()
{
	...
	level.killstreakfuncs["heavy_exosuit"] = ::lib_A7C2::tryuseheavyexosuit;
	...
	level thread lib_A7C2::onplayerconnect();
}

//Function Number: 2
lib_A7C2::tryuseheavyexosuit(param_00,param_01)
{
	...
	var_02 = lib_A7C2::playerlaunchdroppod(param_01);
	return var_02;
}

//Function Number: 3
lib_A7C2::resetweapon()
{
...
//Function Number: 5
lib_A7C2::givejuggernaut(param_00,param_01)
{
	self endon("death");
	...
	if(isdefined(level.var_511A) && level.var_511A)
	{
		self.var_275 = 300 + 25 * self.hordearmor;
	}
...
lib_A7C2::juggernautsounds()
{
	...
	{
		wait(3);
		self playsoundasmaster("juggernaut_breathing_sound");
	}
}

//Function Number: 7
lib_A7C2::radarmover(param_00)
{
	level endon("game_ended");
	self endon("disconnect");
	self endon("jugg_removed");
...
//Function Number: 8
lib_A7C2::juggremover()
{
	level endon("game_ended");
	self endon("disconnect");
	self endon("jugg_removed");
	thread lib_A7C2::juggremoveongameended();
	...
	if(isdefined(self.var_52A3))
	{
		self.var_52A3 destroy();
	}
...
//Function Number: 9
lib_A7C2::juggremoveongameended()
{
	self endon("disconnect");
	self endon("jugg_removed");
...
//Function Number: 11
lib_A7C2::playersetupjuggernautexo(param_00,param_01)
{
	var_02 = spawnstruct();
	self.heavyexodata = var_02;
	var_02.streakplayer = self;
	var_02.hascoopsentry = 1;
	var_02.var_5D57 = param_00;
	...
	if(isdefined(level.var_511A) && level.var_511A)
	{
		self.mechhealth = self.var_275;
	}

	self getnearestnode(1);
	self setclientomnvar("ui_exo_suit_health",1);
	lib_A7C2::playersetjuggexomodel(var_02);
	thread lib_A7C2::playershowjuggernauthud(var_02);
	thread lib_A7C2::playercleanupondeath(var_02);
	thread lib_A7C2::playercleanuponother();
	thread lib_A7C2::playerrocketsandswarmwatcher();
	thread lib_A7C2::playermech_invalid_weapon_watcher();
	thread lib_A7C2::playerhandlebootupsequence();
	thread lib_A7C2::play_goliath_death_fx();
	thread lib_A7C2::playermech_watch_emp_grenade();
	if(isdefined(level.var_511A) && level.var_511A)
	{
	...
	if(var_02.hasradar)
	{
		level thread lib_A7C2::setupradar(self,var_02);
	}

	if(var_02.hasmaniac)
	{
		level thread lib_A7C2::setupmaniac(self);
		lib_A7C2::set_mech_chaingun_state("offline");
	}
	else
	{
		thread lib_A7C2::playerhandlebarrel();
		lib_A7C2::set_mech_chaingun_state("ready");
	}

	if(var_02.haslongpunch)
	{
		level thread lib_A7C2::setuplongpunch(self,var_02);
		lib_A7C2::set_mech_rocket_state("ready");
		thread lib_A7C2::playermech_monitor_rocket_recharge();
	}
	else
	{
		lib_A7C2::set_mech_rocket_state("offline");
		if(!var_02.hasmaniac)
		{
	...
	if(var_02.hastrophy)
	{
		level thread lib_A7C2::setuptrophy(self,var_02);
	}

	if(var_02.hasrockets)
	{
		level thread lib_A7C2::setuprocketswarm(self,var_02);
		lib_A7C2::set_mech_swarm_state("ready");
		thread lib_A7C2::playermech_monitor_swarm_recharge();
	}
	else
	{
		self method_84BF();
		lib_A7C2::set_mech_swarm_state("offline");
	}

	level thread lib_A7C2::delaysetweapon(self);
	if(isdefined(level.var_511A) && level.var_511A)
	{
	...
	if(isdefined(self))
	{
		thread lib_A7C2::self_destruct_goliath();
	}
}

//Function Number: 12
lib_A7C2::playerhandlebootupsequence()
{
...
//Function Number: 13
lib_A7C2::juggernautmodifydamage(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08)
{
	...
	{
		if(isdefined(level.var_511A) && level.var_511A && param_03 == "MOD_TRIGGER_HURT" && param_00 maps\mp\_utility::touchingbadtrigger())
		{
			var_09 = 10000;
		}
		else
	...
	if(var_09 > 0)
	{
		...
		if(isdefined(level.var_511A) && level.var_511A)
		{
			param_00 setclientomnvar("ui_exo_suit_health",param_00.mechhealth / param_00.var_275);
		}
		else
		{
		...
		if(param_00.mechhealth < 0)
		{
			if(isdefined(level.var_511A) && level.var_511A)
			{
				...
				playfx(common_scripts\utility::getfx("goliath_self_destruct"),self.var_2E6,anglestoup(self.var_41));
				self thread [[ level.hordehandlejuggdeath ]]();
			}
			else
			{
				param_00 thread lib_A7C2::playerkillheavyexo(param_05,param_01,param_03,param_04,param_08);
			}
		}
	}
...
//Function Number: 14
lib_A7C2::playerkillheavyexo(param_00,param_01,param_02,param_03,param_04)
{
...
//Function Number: 15
lib_A7C2::delaysetweapon(param_00)
{
	...
	wait 0.05;
	param_00 method_8494(1);
	param_00 disableweaponswitch();
}

//Function Number: 16
lib_A7C2::playercleanupondeath(param_00)
{
	self endon("disconnect");
	self waittill("death",var_01,var_02,var_03);
	if(isdefined(var_01) && isplayer(var_01) && var_01 != self && var_01.inliveplayerkillstreak != self.inliveplayerkillstreak)
	{
	...
	self.var_4E3A = undefined;
	self.mechhealth = undefined;
	lib_A7C2::playerreset(param_00);
}

//Function Number: 17
lib_A7C2::playercleanuponother()
{
	self endon("death");
	self endon("disconnect");
...
lib_A7C2::playerhandlebarrel()
{
	...
	thread lib_A7C2::playercleanupbarrel();
...
//Function Number: 23
lib_A7C2::playerrocketsandswarmwatcher()
{
	self endon("death");
	self endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	for(;;)
	{
		...
		else if(var_00 == "playermech_rocket_swarm_mp" || var_00 == "playermech_rocket_swarm_maniac_mp")
		{
			...
			self waittill("grenade_fire",var_01,var_00);
			self notify("mech_swarm_fire",var_01.var_2E6);
			var_01 delete();
		}
...
//Function Number: 24
lib_A7C2::setupcoopturret(param_00,param_01)
{
	var_02 = param_01 gettagorigin("tag_turret");
	var_03 = lib_A7C2::spawnattachment("juggernaut_sentry_mg_mp","npc_heavy_exo_armor_turret_base",var_02,200,param_01,&"KILLSTREAKS_HEAVY_EXO_SENTRY_LOST");
	var_03 setmode("sentry_offline");
	var_03 setsentryowner(param_01);
	var_03 setleftarc(180);
	var_03 setrightarc(180);
	var_03 method_8157(55);
	...
	if(level.teambased)
	{
		var_03 setturretteam(param_01.inliveplayerkillstreak);
	}

	var_03 common_scripts\utility::make_entity_sentient_mp(param_01.inliveplayerkillstreak);
	...
	var_03.effect = lib_A7C2::spawnattachmenteffect(var_02,param_01);
	var_03.effect linkto(var_03,"tag_player",(29,-7,-6),(0,0,0));
	var_03.effect hide();
	param_00.coopturret = var_03;
	thread lib_A7C2::stopturret(param_00,var_03,param_01);
	thread lib_A7C2::handlecoopshooting(param_00,var_03,param_01);
	thread lib_A7C2::handleturretonplayerdone(param_00,var_03,param_01);
	return var_03;
}

//Function Number: 25
lib_A7C2::stopturret(param_00,param_01,param_02)
{
	...
	if(isdefined(param_01))
	{
		...
		lib_A7C2::removecoopturretbuddy(param_00);
		lib_A7C2::stopfxonattachment(param_01,common_scripts\utility::getfx("green_light_mp"),1);
		param_01 playsoundasmaster("sentry_explode");
		...
		param_01 method_8495(0);
		param_01 sentient_freeentitysentient();
		param_01 setdefaultdroppitch(35);
		param_01 setsentryowner(undefined);
		level thread lib_A7C2::doturretdeatheffects(param_01);
	}
}

//Function Number: 26
lib_A7C2::handlecoopshooting(param_00,param_01,param_02)
{
	...
	for(;;)
	{
		...
		if(param_01.owner attackbuttonpressed() && !param_01 method_844F())
		{
			param_01 lib_A7C2::turretshootblank(param_01.baseowner);
			wait(var_03);
			continue;
		}
...
//Function Number: 27
lib_A7C2::turretshoot()
{
	self shootturret();
	lib_A7C2::turretshootblank(self.baseowner);
}

//Function Number: 28
lib_A7C2::turretshootblank(param_00)
{
	...
	function_02A0(var_01,var_03,"juggernaut_sentry_mg_mp",var_04,param_00);
}

//Function Number: 29
lib_A7C2::doturretdeatheffects(param_00)
{
	param_00 playsoundasmaster("sentry_explode");
	...
	param_00 playsoundasmaster("sentry_explode_smoke");
...
//Function Number: 30
lib_A7C2::handleturretonplayerdone(param_00,param_01,param_02)
{
	thread lib_A7C2::attachmentdeath(param_00,param_01,param_02);
	lib_A7C2::waittillattachmentdone(param_02);
	lib_A7C2::stopfxonattachment(param_01,common_scripts\utility::getfx("green_light_mp"));
	param_01 maps\mp\killstreaks\_autosentry::removefromturretlist(param_01 getentitynumber());
	param_01.issentry = 0;
	param_02 notify("turretDead");
	lib_A7C2::removecoopturretbuddy(param_00);
...
//Function Number: 31
lib_A7C2::setupradar(param_00,param_01)
{
	var_02 = param_00 gettagorigin("tag_recon_back");
	var_03 = lib_A7C2::spawnattachment("radar","npc_heavy_exo_armor_recon_back_base",var_02,undefined,param_00);
	var_03 linkto(param_00,"tag_recon_back",(0,0,0),(0,0,0));
	param_00 thread lib_A7C2::playerhandleradarping(param_01,var_03);
	lib_A7C2::waittillattachmentdone(param_00);
	wait 0.05;
	var_03 delete();
}

//Function Number: 32
lib_A7C2::playerhandleradarping(param_00,param_01)
{
	param_01 endon("death");
	self endon("death");
	self endon("disconnect");
	...
	for(;;)
	{
		...
		lib_A7C2::activate_exo_ping();
		self setclientomnvar("ui_exo_suit_recon_cd",1);
		wait(10);
		lib_A7C2::deactivate_exo_ping();
		lib_A7C2::waitattachmentcooldown(5,"ui_exo_suit_recon_cd");
	}
}
...
lib_A7C2::activate_exo_ping()
{
	thread lib_A7C2::stop_exo_ping();
	self setperk("specialty_exo_ping",1,0);
	self method_82F4("mp_exo_cloak_activate");
...
lib_A7C2::deactivate_exo_ping()
{
	...
	self method_82F4("mp_exo_cloak_deactivate");
...
//Function Number: 36
lib_A7C2::setupmaniac(param_00)
{
	var_01 = param_00 gettagorigin("tag_maniac_l");
	var_02 = lib_A7C2::spawnattachment("speedAttachment","npc_heavy_exo_armor_maniac_l_base",var_01,undefined,param_00);
	var_02 linkto(param_00,"tag_maniac_l",(0,0,0),(0,0,0));
	var_01 = param_00 gettagorigin("tag_maniac_r");
	var_03 = lib_A7C2::spawnattachment("speedAttachment","npc_heavy_exo_armor_maniac_r_base",var_01,undefined,param_00);
	var_03 linkto(param_00,"tag_maniac_r",(0,0,0),(0,0,0));
	var_04 = param_00 gettagorigin("tag_jetpack");
	var_05 = lib_A7C2::spawnattachment("speedAttachment","npc_heavy_exo_armor_jetpack_base",var_04,undefined,param_00);
	var_05 linkto(param_00,"tag_jetpack",(0,0,0),(0,0,0));
	lib_A7C2::waittillattachmentdone(param_00);
	lib_A7C2::attachmentexplode(var_02,param_00,"maniac",var_03);
	lib_A7C2::attachmentexplode(var_05,param_00,"maniac");
	wait 0.05;
	var_02 delete();
	var_03 delete();
	var_05 delete();
...
//Function Number: 37
lib_A7C2::setuplongpunch(param_00,param_01)
{
	param_00 setoffhandprimaryclass("playermech_rocket_mp");
	param_00 giveweapon("playermech_rocket_mp");
	var_02 = "tag_origin";
	param_00 thread lib_A7C2::playerwatchnoobtubeuse(param_01);
	lib_A7C2::waittillattachmentdone(param_00);
}

//Function Number: 38
lib_A7C2::playerwatchnoobtubeuse(param_00)
{
	self endon("death");
	self endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	for(;;)
	{
		...
		thread lib_A7C2::reloadrocket(self,param_00);
	}
}

//Function Number: 39
lib_A7C2::reloadrocket(param_00,param_01)
{
	param_00 endon("death");
	param_00 endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	lib_A7C2::waitattachmentcooldown(10,"ui_exo_suit_punch_cd");
}

//Function Number: 40
lib_A7C2::playrocketreloadsound(param_00)
{
	self method_82F4("orbitalsupport_reload_40mm");
}

//Function Number: 41
lib_A7C2::waitattachmentcooldown(param_00,param_01)
{
	var_02 = 0;
	for(;;)
	{
...
//Function Number: 42
lib_A7C2::setuptrophy(param_00,param_01)
{
	var_02 = param_00 gettagorigin("j_spine4");
	var_03 = lib_A7C2::spawnattachment("trophy","npc_heavy_exo_armor_trophy_l_base",var_02,undefined,param_00);
	var_03.stunned = 0;
	var_03.ammo = 1;
	var_03 linkto(param_00,"tag_trophy_l",(0,0,0),(0,0,0));
	var_03.fx_origin = "heavy_exo_trophy_mp";
	var_03 thread maps\mp\gametypes\_equipment::trophyactive(param_00,undefined,1,var_03.fx_origin);
	...
	var_04 = lib_A7C2::spawnattachment("trophy","npc_heavy_exo_armor_trophy_r_base",var_02,undefined,param_00);
	var_04.stunned = 0;
	var_04.ammo = 1;
	var_04 linkto(param_00,"tag_trophy_r",(0,0,0),(0,0,0));
	var_04.fx_origin = "heavy_exo_trophy_mp";
	var_04 thread maps\mp\gametypes\_equipment::trophyactive(param_00,undefined,1,var_04.fx_origin);
...
lib_A7C2::trophystunbegin()
{
	...
	thread lib_A7C2::trophymovestunent(var_00);
...
//Function Number: 44
lib_A7C2::trophymovestunent(param_00)
{
	self endon("death");
	self endon("stunEnd");
	for(;;)
	{
		param_00.var_2E6 = self.var_2E6;
		wait 0.05;
	}
}

//Function Number: 45
lib_A7C2::setuprocketswarm(param_00,param_01)
{
	...
	param_00 setoffhandsecondaryclass(var_02);
	param_00 giveweapon(var_02);
	var_03 = "tag_origin";
	var_04 = param_00 gettagorigin(var_03);
	var_05 = lib_A7C2::spawnattachment("rocketAttachment","npc_heavy_exo_armor_missile_pack_base",var_04,undefined,param_00);
	...
	thread lib_A7C2::scanforrocketenemies(var_05,param_00);
	param_00 thread lib_A7C2::playerwatchrocketuse(var_05,param_01);
	lib_A7C2::waittillattachmentdone(param_00,var_05);
	wait 0.05;
	var_05 delete();
...
//Function Number: 46
lib_A7C2::scanforrocketenemies(param_00,param_01)
{
	param_01 endon("death");
	...
	for(;;)
	{
		...
		var_02 = lib_A7C2::getbestenemy(param_01,4);
		if(isdefined(var_02))
		{
			if(!isdefined(param_00.enemytarget) || param_00.enemytarget != var_02)
			{
				thread lib_A7C2::markplayerasrockettarget(param_00,param_01,var_02);
			}
...
lib_A7C2::getbestenemy(param_00,param_01)
{
	...
	{
		if(var_08.inliveplayerkillstreak == param_00.inliveplayerkillstreak)
		{
			continue;
		}
	...
	{
		var_0E = lib_A7C2::gethighestdot(var_06);
		var_0E.checked = 1;
		var_0F = var_04;
...
//Function Number: 50
lib_A7C2::gethighestdot(param_00)
{
	if(param_00.size == 0)
	{
		return;
...
//Function Number: 51
lib_A7C2::playerwatchrocketuse(param_00,param_01)
{
	param_00 endon("death");
	for(;;)
	{
		...
		thread lib_A7C2::handlelockedtarget(param_00,param_01);
		thread lib_A7C2::reloadrocketswarm(param_00,self,param_01);
		thread lib_A7C2::firerocketswarm(param_00,self,var_02);
	}
}

//Function Number: 52
lib_A7C2::handlelockedtarget(param_00,param_01)
{
	param_00 endon("death");
	param_00.lockedtarget = 1;
	param_00 notify("lockedTarget");
	lib_A7C2::waittillrocketsexploded(param_00);
	if(isdefined(param_00))
	{
...
//Function Number: 53
lib_A7C2::firerocketswarm(param_00,param_01,param_02)
{
	...
	for(var_06 = 0;var_06 < 4;var_06++)
	{
		...
		var_08 = var_03 + lib_A7C2::random_vector(0.2);
		...
		var_09 thread lib_A7C2::rockettargetent(param_00,param_00.enemytarget,var_05[var_06]);
		var_09 thread lib_A7C2::rocketdestroyaftertime(7);
	}
}

//Function Number: 54
lib_A7C2::rockettargetent(param_00,param_01,param_02)
{
	param_00 endon("death");
	if(isdefined(param_01))
	{
		self missilesettargetent(param_01,param_02);
	}
...
//Function Number: 55
lib_A7C2::rocketdestroyaftertime(param_00)
{
	self endon("death");
	wait(param_00);
	self delete();
...
//Function Number: 56
lib_A7C2::reloadrocketswarm(param_00,param_01,param_02)
{
	param_00 endon("death");
	param_00.reloading = 1;
	lib_A7C2::waitattachmentcooldown(10,"ui_exo_suit_rockets_cd");
	param_00.reloading = 0;
}

//Function Number: 57
lib_A7C2::playrocketswarmreloadsound(param_00,param_01,param_02)
{
	param_00 endon("death");
	var_03 = 3;
...
//Function Number: 58
lib_A7C2::markplayerasrockettarget(param_00,param_01,param_02)
{
	...
	thread lib_A7C2::cleanuprockettargeticon(param_00,param_02,param_01);
	...
	if(isdefined(level.var_511A) && level.var_511A)
	{
		...
		{
			if(level.objdefend && distancesquared(param_01.var_2E6,level.currentdefendloc.var_2E6) > 640000)
			{
...
//Function Number: 59
lib_A7C2::cleanuprockettargeticon(param_00,param_01,param_02)
{
	param_01 endon("disconnect");
	lib_A7C2::waittillunmarkplayerasrockettarget(param_00);
	if(isdefined(level.var_511A) && level.var_511A && isdefined(param_02))
	{
		if(level.currentaliveenemycount < 3)
		{
			if(level.objdefend && distancesquared(param_02.var_2E6,level.currentdefendloc.var_2E6) > 640000)
			{
...
//Function Number: 62
lib_A7C2::waittillattachmentdone(param_00,param_01,param_02)
{
	param_00 endon("disconnect");
	param_00 endon("death");
	if(isdefined(level.var_511A) && level.var_511A)
...
//Function Number: 63
lib_A7C2::delayplayfx(param_00,param_01)
{
	param_00 endon("death");
	wait 0.05;
	wait 0.05;
...
//Function Number: 64
lib_A7C2::stopfxonattachment(param_00,param_01,param_02)
{
	if(isdefined(param_00.effect))
	{
		...
		{
			playfx(common_scripts\utility::getfx("juggernaut_sparks"),param_00.effect.var_2E6);
		}
...
//Function Number: 65
lib_A7C2::attachmentdeath(param_00,param_01,param_02,param_03)
{
	param_02 endon("death");
	param_02 endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
...
//Function Number: 66
lib_A7C2::attachmentexplode(param_00,param_01,param_02,param_03)
{
	if(isdefined(param_01))
	{
		if(isalive(param_01))
		{
			param_01 thread lib_A7C2::playerplayattachmentdialog(param_00.attachmenttype);
		}

		if(isdefined(param_00))
		{
			playfx(common_scripts\utility::getfx("juggernaut_sparks"),param_00.var_2E6);
		}

		if(isdefined(param_03))
		{
			playfx(common_scripts\utility::getfx("juggernaut_sparks"),param_03.var_2E6);
		}
...
//Function Number: 67
lib_A7C2::hidefromplayer(param_00)
{
	self hide();
...
//Function Number: 68
lib_A7C2::hidefromplayers(param_00)
{
	self hide();
	foreach(var_02 in level.var_328)
	{
...
//Function Number: 69
lib_A7C2::spawnattachment(param_00,param_01,param_02,param_03,param_04,param_05)
{
	var_06 = undefined;
	...
	if(isdefined(param_03))
	{
		...
		var_06.var_275 = var_06.health;
		var_06.damagecallback = ::lib_A7C2::handleattachmentdamage;
		if(isdefined(param_05))
		{
			var_06 thread lib_A7C2::handleattachmentdeath(param_00,param_04,param_05);
		}

		var_06 method_8495(1);
	}

	var_06 lib_A7C2::hidefromplayer(param_04);
	var_06.owner = param_04;
	if(level.teambased)
	{
		var_06.inliveplayerkillstreak = param_04.inliveplayerkillstreak;
	}
...
//Function Number: 70
lib_A7C2::spawnattachmenteffect(param_00,param_01,param_02)
{
	if(!isdefined(param_02))
	{
		param_02 = 0;
	...
	var_03 lib_A7C2::hidefromplayer(param_01);
	thread lib_A7C2::delayplayfx(var_03,common_scripts\utility::getfx("green_light_mp"));
	return var_03;
}

//Function Number: 71
lib_A7C2::handleattachmentdeath(param_00,param_01,param_02)
{
...
//Function Number: 72
lib_A7C2::handleattachmentdamage(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09,param_0A,param_0B)
{
...
//Function Number: 73
lib_A7C2::random_vector(param_00)
{
	return (randomfloat(param_00) - param_00 * 0.5,randomfloat(param_00) - param_00 * 0.5,randomfloat(param_00) - param_00 * 0.5);
}

//Function Number: 74
lib_A7C2::handlecoopjoining(param_00,param_01)
{
	for(;;)
	{
		var_02 = maps\mp\killstreaks\_coop_util::promptforstreaksupport(param_01.inliveplayerkillstreak,&"MP_JOIN_HEAVY_EXO","heavy_exosuit_coop_offensive","assist_mp_goliath","copilot_mp_goliath",param_01);
		level thread lib_A7C2::watchforjoin(var_02,param_01,param_00);
		var_03 = lib_A7C2::waittillpromptcomplete(param_01,"buddyJoinedStreak");
		...
		var_03 = lib_A7C2::waittillpromptcomplete(param_01,"buddyLeftCoopTurret");
		if(!isdefined(var_03))
		{
			return;
		}
...
//Function Number: 75
lib_A7C2::waittillpromptcomplete(param_00,param_01,param_02,param_03)
{
	param_00 endon("death");
	param_00 endon("disconnect");
...
//Function Number: 76
lib_A7C2::waittillturretstuncomplete(param_00,param_01)
{
	param_01 endon("death");
	param_01 endon("disconnect");
...
//Function Number: 77
lib_A7C2::watchforjoin(param_00,param_01,param_02)
{
	param_01 endon("disconnect");
	param_01 endon("death");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	var_03 thread lib_A7C2::playerremotecoopturret(param_02);
}

//Function Number: 78
lib_A7C2::playerremotecoopturret(param_00)
{
	self endon("disconnect");
...
//Function Number: 81
lib_A7C2::playershowjuggernauthud(param_00)
{
	param_00.hud = [];
	thread lib_A7C2::playerwatchemp(param_00);
	lib_A7C2::createjuggernautoverlay(param_00);
}
...
lib_A7C2::createjuggernautoverlay(param_00)
{
	...
	thread lib_A7C2::playermech_state_manager();
}

//Function Number: 83
lib_A7C2::playerwatchemp(param_00)
{
	self endon("death");
	self endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	for(;;)
	{
		...
		for(;;)
		{
			...
			wait 0.05;
			if(lib_A7C2::playershouldshowhud())
			{
				break;
			}
		}
...
//Function Number: 86
lib_A7C2::playerlaunchdroppod(param_00)
{
	...
	thread lib_A7C2::firedroppod(var_01,param_00);
	return 1;
}
...
droppodmovenearbyallies(param_00)
{
	...
	self.unresolved_collision_nodes = getnodesinradius(self.var_2E6,300,80,200);
	foreach(var_02 in level.characters)
	{
		...
		if(function_0285(var_02,param_00))
		{
			if(distancesquared(self.var_2E6,var_02.var_2E6) < 6000)
			{
...
//Function Number: 89
lib_A7C2::firedroppod(param_00,param_01)
{
	...
	var_04.killcament method_8446(var_04,"tag_origin",(0,0,200),(0,10,10));
	...
	if(distancesquared(var_04.var_2E6,var_03) > 22500)
	{
		var_07 delete();
	...
	var_03 = getgroundposition(var_04.var_2E6 + (0,0,8),20);
	...
	var_09 thread lib_A7C2::control_goliath_usability();
	...
	var_09 thread lib_A7C2::usegoliathupdater();
	var_0B = spawnstruct();
	var_0B.zombiesstarted = var_09;
	var_0B.playdeathfx = 1;
	var_0B.deathoverridecallback = ::lib_A7C2::movingplatformdeathfunc;
	var_0B.var_9403 = ::lib_A7C2::movingplatformtouchvalid;
	var_09 thread maps\mp\_movers::handle_moving_platforms(var_0B);
	var_09 thread lib_A7C2::handle_goliath_drop_pod_timeout(var_08);
	...
	if(isdefined(level.var_511A) && level.var_511A)
	{
		if(level.var_A3D0 || level.var_91E7["allies"])
		{
			var_09 lib_A7C2::deletegoliathpod();
		}
		else
		{
	...
	var_0C = var_09 lib_A7C2::playerwaittillgoliathactivated();
	if(isdefined(level.var_511A) && level.var_511A && isdefined(self))
	{
	...
	if(isdefined(var_0C) && isalive(var_0C))
	{
		...
		if(isdefined(var_09))
		{
			var_09 lib_A7C2::deletegoliathpod(0);
		}

		playfx(level._effect["jugg_droppod_open"],var_03,var_10);
		wait(0.1);
		var_0C lib_A7C2::is_entering_goliath(var_11,var_03);
		if(isdefined(var_0C) && isalive(var_0C) && !isdefined(level.var_511A) && level.var_511A && isdefined(var_0C.var_51B2) && var_0C.var_51B2)
		{
			var_0C setorigin(var_03,1);
			var_0C setplayerangles(var_11.var_41);
			var_0C enableweapons();
			var_0C lib_A7C2::givejuggernaut("juggernaut_exosuit",param_01);
			var_11 delete();
			var_0C method_8517();
			if(isdefined(level.var_511A) && level.var_511A)
			{
...
destroy_nearby_turrets(param_00)
{
	...
	{
		if(isdefined(var_03.turret) && distancesquared(var_03.turret.var_2E6,param_00) <= var_01)
		{
...
//Function Number: 92
lib_A7C2::movingplatformdeathfunc(param_00)
{
	...
	if(isdefined(param_00.zombiesstarted))
	{
		param_00.zombiesstarted delete();
	}
}

//Function Number: 93
lib_A7C2::movingplatformtouchvalid(param_00)
{
	return lib_A7C2::goliathandcarepackagevalid(param_00) && lib_A7C2::goliathandgoliathvalid(param_00) && lib_A7C2::goliathandplatformvalid(param_00);
}

//Function Number: 94
lib_A7C2::goliathandcarepackagevalid(param_00)
{
	return !isdefined(self.cratetype) || !isdefined(param_00.targetname) || self.cratetype != "juggernaut" || param_00.targetname != "care_package";
}

//Function Number: 95
lib_A7C2::goliathandgoliathvalid(param_00)
{
	return !isdefined(self.cratetype) || !isdefined(param_00.cratetype) || self.cratetype != "juggernaut" || param_00.cratetype != "juggernaut";
}

//Function Number: 96
lib_A7C2::goliathandplatformvalid(param_00)
{
...
//Function Number: 97
lib_A7C2::control_goliath_usability()
{
	self endon("captured");
	self endon("death");
	level endon("game_ended");
	...
	for(;;)
	{
		...
		{
			var_04 = 0;
			if(var_01 isonground() && !var_01 method_801A() && !var_01 method_83B3() && !var_01 ismantling() && maps\mp\_utility::isreallyalive(var_01) && var_01 getstance() == "stand")
			{
				if(distancesquared(self.var_2E6,var_01.var_2E6) < 6000)
				{
					if(var_01 worldpointinreticle_rect(self.var_2E6 + (0,0,50),65,400,600))
					{
						var_04 = 1;
					}
				}
...
//Function Number: 98
lib_A7C2::is_entering_goliath(param_00,param_01)
{
...
//Function Number: 99
lib_A7C2::createcollision(param_00)
{
...
//Function Number: 100
lib_A7C2::playerwaittillgoliathactivated()
{
	self endon("death");
	self waittill("captured",var_00);
	var_00 setstance("stand");
	var_00 getnearestnode(1);
	if(isdefined(self.owner) && var_00 != self.owner)
	{
		if(!level.teambased || var_00.inliveplayerkillstreak != self.owner.inliveplayerkillstreak)
		{
			var_00 thread maps\mp\_events::hijackerevent(self.owner);
		}
		else if(!isdefined(level.var_511A))
		{
...
lib_A7C2::usegoliathupdater()
{
	...
	foreach(var_01 in level.var_328)
	{
		...
		{
			self disableplayeruse(var_01);
			thread lib_A7C2::usepostjuggernautupdater(var_01);
		}
	}
...
//Function Number: 102
lib_A7C2::usepostjuggernautupdater(param_00)
{
	self endon("death");
	level endon("game_ended");
	param_00 endon("disconnect");
...
//Function Number: 103
lib_A7C2::adjustlink(param_00,param_01,param_02,param_03,param_04)
{
	param_00 endon("death");
	if(!isdefined(param_03))
	{
	...
	thread lib_A7C2::drawspine(param_02,param_00);
...
//Function Number: 104
lib_A7C2::drawspine(param_00,param_01)
{
	param_00 endon("disconnect");
	param_00 endon("death");
	param_01 endon("death");
	...
	for(;;)
	{
		...
		var_03 = param_01.var_41;
		lib_A7C2::debug_axis(var_02,var_03);
		wait 0.05;
	}
}

//Function Number: 105
lib_A7C2::debug_axis(param_00,param_01)
{
	var_02 = 20;
...
//Function Number: 106
lib_A7C2::playermech_ui_state_reset()
{
...
//Function Number: 107
lib_A7C2::playermech_state_manager()
{
	self endon("disconnect");
	...
	lib_A7C2::playermech_ui_state_reset();
	lib_A7C2::set_mech_state();
	lib_A7C2::set_mech_chaingun_state();
	lib_A7C2::set_mech_rocket_state();
	lib_A7C2::set_mech_swarm_state();
	wait 0.05;
	for(;;)
	{
		lib_A7C2::state_chaingun_pump();
		lib_A7C2::state_rocket_pump();
		lib_A7C2::state_swarm_pump();
		lib_A7C2::playermech_ui_update_lui(self.mechuistate);
		wait(0.05);
	}
}

//Function Number: 108
lib_A7C2::set_mech_state(param_00)
{
	if(!isdefined(param_00))
	{
		param_00 = "none";
...
//Function Number: 111
lib_A7C2::get_front_sorted_threat_list(param_00,param_01)
{
	var_02 = [];
	foreach(var_04 in param_00)
	{
		if(vectordot(var_04.var_2E6 - self.var_2E6,param_01) < 0)
		{
			continue;
		}
...
//Function Number: 112
lib_A7C2::playermech_ui_weapon_feedback(param_00,param_01)
{
	self endon("disconnect");
	self endon("exit_mech");
	if(isdefined(level.var_511A) && level.var_511A)
...
//Function Number: 113
lib_A7C2::playermech_ui_update_lui(param_00)
{
	var_01 = lib_A7C2::playerisrocketswarmtargetlocked();
	var_02 = 0;
	if(var_01)
	{
		var_02 = 1;
...
//Function Number: 114
lib_A7C2::playermech_invalid_gun_callback()
{
...
//Function Number: 115
lib_A7C2::playermech_invalid_rocket_callback()
{
...
//Function Number: 116
lib_A7C2::playermech_invalid_swarm_callback()
{
...
//Function Number: 117
lib_A7C2::playermech_invalid_weapon_instance(param_00,param_01)
{
	self endon("disconnect");
	self endon("exit_mech");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	for(;;)
	{
		...
		if(self [[ param_00 ]]())
		{
			if(!var_02)
			{
				...
				{
					var_02 = 1;
					self method_82F4("wpn_mech_offline");
					wait(1.5);
				}
			}
...
//Function Number: 118
lib_A7C2::playermech_invalid_weapon_watcher()
{
	thread lib_A7C2::playermech_invalid_weapon_instance(::attackbuttonpressed,::lib_A7C2::playermech_invalid_gun_callback);
	thread lib_A7C2::playermech_invalid_weapon_instance(::fragbuttonpressed,::lib_A7C2::playermech_invalid_rocket_callback);
	thread lib_A7C2::playermech_invalid_weapon_instance(::secondaryoffhandbuttonpressed,::lib_A7C2::playermech_invalid_swarm_callback);
}

//Function Number: 119
lib_A7C2::state_main_pump()
{
	switch(lib_A7C2::get_mech_state())
	{
		case "dmg2":
...
lib_A7C2::state_chaingun_pump()
{
	var_00 = lib_A7C2::get_mech_chaingun_state();
	var_01 = self getcurrentweapon();
	self.mechuistate.chaingun.heatlevel = self method_83B9(var_01);
	self.mechuistate.chaingun.overheated = self method_83BA(var_01);
	if(var_00 == "ready")
	{
		if(self.mechuistate.chaingun.overheated)
		{
			lib_A7C2::set_mech_chaingun_state("overheat");
			return;
		}

		if(self attackbuttonpressed())
		{
			lib_A7C2::set_mech_chaingun_state("firing");
			return;
		}
...
lib_A7C2::state_rocket_pump()
{
	var_00 = lib_A7C2::get_mech_rocket_state();
	if(var_00 != "offline" && lib_A7C2::playermech_invalid_rocket_callback())
	{
		lib_A7C2::set_mech_rocket_state("reload");
		return;
	}

	if(var_00 == "reload" && !lib_A7C2::playermech_invalid_rocket_callback())
	{
		lib_A7C2::set_mech_rocket_state("ready");
	}
}
...
lib_A7C2::state_swarm_pump()
{
	var_00 = lib_A7C2::get_mech_swarm_state();
	if(!lib_A7C2::playerisrocketswarmtargetlocked() && !lib_A7C2::playerisrocketswarmreloading())
	{
		lib_A7C2::set_mech_swarm_state("target");
		return;
	}

	if(var_00 == "target" && lib_A7C2::playermech_invalid_swarm_callback())
	{
		lib_A7C2::set_mech_swarm_state("reload");
		return;
	}

	if(var_00 == "reload" && !lib_A7C2::playermech_invalid_swarm_callback())
	{
		lib_A7C2::set_mech_swarm_state("ready");
		return;
	}
...
//Function Number: 123
lib_A7C2::set_mech_chaingun_state(param_00)
{
	if(!isdefined(param_00))
	{
		param_00 = "none";
...
//Function Number: 124
lib_A7C2::get_mech_chaingun_state()
{
	if(!isdefined(self.mechuistate))
	{
		return;
...
//Function Number: 125
lib_A7C2::same_mech_chaingun_last_state()
{
...
//Function Number: 126
lib_A7C2::set_mech_rocket_state(param_00)
{
	if(!isdefined(param_00))
	{
		param_00 = "none";
...
//Function Number: 127
lib_A7C2::get_mech_rocket_state()
{
	if(!isdefined(self.mechuistate))
	{
		return;
...
//Function Number: 128
lib_A7C2::same_mech_rocket_last_state()
{
...
//Function Number: 129
lib_A7C2::set_mech_swarm_state(param_00)
{
	if(!isdefined(param_00))
	{
		param_00 = "none";
...
//Function Number: 131
lib_A7C2::same_mech_swarm_last_state()
{
...
//Function Number: 132
lib_A7C2::playermech_monitor_update_recharge(param_00,param_01)
{
	param_00.recharge = 0;
...
//Function Number: 133
lib_A7C2::playermech_monitor_rocket_recharge()
{
	self endon("disconnect");
	self endon("exit_mech");
	...
	for(;;)
	{
		...
		self disableoffhandweapons();
		lib_A7C2::playermech_monitor_update_recharge(self.mechuistate.rocket,10);
		self enableoffhandweapons();
		wait(0.05);
	}
}
...
//Function Number: 134
lib_A7C2::playermech_monitor_swarm_recharge()
{
	self endon("disconnect");
	self endon("exit_mech");
	...
	for(;;)
	{
		...
		self method_84BF();
		lib_A7C2::playermech_monitor_update_recharge(self.mechuistate.swarm,10);
		self method_84C0();
		wait(0.05);
	}
}
...
//Function Number: 135
lib_A7C2::play_goliath_death_fx()
{
	level endon("game_ended");
	self endon("disconnect");
	if(isdefined(level.var_511A) && level.var_511A)
	...
	else if(!isdefined(self.juggernautsuicide) && !isdefined(level.var_511A))
	{
...
//Function Number: 136
lib_A7C2::self_destruct_goliath()
{
	level endon("game_ended");
	self endon("death");
	self endon("disconnect");
	...
	while(maps\mp\_utility::isjuggernaut())
	{
		if(self usebuttonpressed())
		{
			...
			if(var_00 > 1)
			{
				...
				radiusdamage(self.var_2E6 + (0,0,50),400,200,20,self,"MOD_EXPLOSIVE","killstreak_goliathsd_mp");
				if(isdefined(level.var_511A) && level.var_511A)
				{
					self thread [[ level.hordehandlejuggdeath ]]();
				}
...
playermechtimeout()
{
	...
	{
		wait(1);
		self.mechhealth = self.mechhealth - int(self.var_275 / 100);
		...
		self setclientomnvar("ui_exo_suit_health",self.mechhealth / self.var_275);
	}
}

//Function Number: 138
lib_A7C2::onplayerconnect()
{
	for(;;)
	{
		level waittill("connected",var_00);
		var_00 thread lib_A7C2::onplayerspawned();
	}
}

//Function Number: 139
lib_A7C2::onplayerspawned()
{
	self endon("disconnect");
	for(;;)
	{
...
//Function Number: 140
lib_A7C2::deletegoliathpod(param_00,param_01)
{
	if(!isdefined(param_00))
	{
		param_00 = 1;
...
//Function Number: 141
lib_A7C2::handle_goliath_drop_pod_timeout(param_00)
{
	level endon("game_ended");
	self endon("death");
	wait(120);
	if(isdefined(level.var_511A) && level.var_511A && param_00)
	{
...
//Function Number: 143
lib_A7C2::handle_goliath_drop_pod_removal(param_00)
{
	level endon("game_ended");
	self endon("death");
	param_00 waittill("death");
...
//Function Number: 144
lib_A7C2::playermech_watch_emp_grenade()
{
	level endon("game_ended");
	self endon("death");
	self endon("disconnect");
	...
	for(;;)
	{
		...
		{
			var_00 thread lib_A7C2::ch_emp_goliath_think();
		}
	}
}

//Function Number: 145
lib_A7C2::ch_emp_goliath_think()
{
	level endon("game_ended");
	self endon("death");
	self endon("disconnect");
