...
lib_A7A1::init()
{
	...
	level thread lib_A7A1::onplayerconnect();
	level thread lib_A7A1::onlastalive();
	level thread lib_A7A1::musiccontroller();
	level thread lib_A7A1::ongameended();
	level thread lib_A7A1::onroundswitch();
}

//Function Number: 2
lib_A7A1::onplayerconnect()
{
	for(;;)
	{
		level waittill("connected",var_00);
		var_00 thread lib_A7A1::onplayerspawned();
		var_00 thread lib_A7A1::finalkillcammusic();
	}
}
...
lib_A7A1::onplayerspawned()
{
	...
	if(getdvar("virtuallobbyactive") == "0")
	{
		if(!level.splitscreen || level.splitscreen && !isdefined(level.playedstartingmusic))
		{
			...
			{
				self method_82F4(game["music"]["spawn_" + self.inliveplayerkillstreak]);
			}
		...
		if(isdefined(game["dialog"]["gametype"]) && !level.splitscreen || self == level.var_328[0])
		{
			if(isdefined(game["dialog"]["allies_gametype"]) && self.inliveplayerkillstreak == "allies")
			{
				maps\mp\_utility::leaderdialogonplayer("allies_gametype");
			}
			else if(isdefined(game["dialog"]["axis_gametype"]) && self.inliveplayerkillstreak == "axis")
			{
				maps\mp\_utility::leaderdialogonplayer("axis_gametype");
			}
			else if(!self issplitscreenplayer() || self method_82E8())
			{
		...
		if(self.inliveplayerkillstreak == game["attackers"])
		{
			if(!self issplitscreenplayer() || self method_82E8())
			{
...
//Function Number: 9
lib_A7A1::ongameended()
{
	level thread lib_A7A1::roundwinnerdialog();
	level thread lib_A7A1::gamewinnerdialog();
	...
	{
		if(var_02 issplitscreenplayer() && !var_02 method_82E8())
		{
			continue;
		}

		if(var_02.connectedpostgame["team"] != "allies" && var_02.connectedpostgame["team"] != "axis")
		{
			var_02 method_82F4(game["music"]["nuke_music"]);
			continue;
		}

		if(isdefined(var_00) && isplayer(var_00) && var_02 == var_00)
		{
			var_02 method_82F4(game["music"]["victory_" + var_02.connectedpostgame["team"]]);
			continue;
		}

		if(!level.splitscreen)
		{
			var_02 method_82F4(game["music"]["defeat_" + var_02.connectedpostgame["team"]]);
		}
	}
}

//Function Number: 10
lib_A7A1::roundwinnerdialog()
{
...
//Function Number: 12
lib_A7A1::musiccontroller()
{
	level endon("game_ended");
	if(!level.hardcoremode && getdvar("virtualLobbyActive") == "0")
	{
		thread lib_A7A1::suspensemusic();
	}
		...
		if(!level.splitscreen)
		{
			...
			else if(var_00 == "score")
			{
				...
				else
				{
					...
					if(!level.hardcoremode)
					{
						var_01 method_82F4(game["music"]["winning_" + var_01.connectedpostgame["team"]]);
						foreach(var_05 in level.var_328)
						{
							...
							var_05 method_82F4(game["music"]["losing_" + var_05.connectedpostgame["team"]]);
						}
					}
