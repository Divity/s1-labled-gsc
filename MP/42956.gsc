...
//Function Number: 2
lib_A7CC::giveplaceable(param_00)
{
	var_01 = lib_A7CC::createplaceable(param_00);
	lib_A7CC::removeperks();
	self.carrieditem = var_01;
	var_02 = lib_A7CC::onbegincarrying(param_00,var_01,1);
	self.carrieditem = undefined;
	lib_A7CC::restoreperks();
	return isdefined(var_01);
}

//Function Number: 3
lib_A7CC::createplaceable(param_00)
{
	...
	var_02 lib_A7CC::deactivate(param_00);
	var_02 thread lib_A7CC::timeout(param_00);
	var_02 thread lib_A7CC::handleuse(param_00);
	var_02 thread lib_A7CC::onkillstreakdisowned(param_00);
	var_02 thread lib_A7CC::ongameended(param_00);
	var_02 thread lib_A7CC::createbombsquadmodel(param_00);
	return var_02;
}

//Function Number: 4
lib_A7CC::handleuse(param_00)
{
	self endon("death");
	...
	for(;;)
	{
		...
		var_01 lib_A7CC::onbegincarrying(param_00,self,0);
	}
}

//Function Number: 5
lib_A7CC::onbegincarrying(param_00,param_01,param_02)
{
	self endon("death");
	self endon("disconnect");
	param_01 thread lib_A7CC::oncarried(param_00,self);
	...
	for(;;)
	{
		...
		{
			param_01 lib_A7CC::oncancel(param_00,var_03 == "force_cancel_placement" && !isdefined(param_01.firstplacement));
			return 0;
		}
		else if(param_01.canbeplaced)
		{
			param_01 thread lib_A7CC::onplaced(param_00);
			common_scripts\utility::_enableweapon();
			return 1;
		}
	}
...
//Function Number: 6
lib_A7CC::oncancel(param_00,param_01)
{
...
//Function Number: 7
lib_A7CC::onplaced(param_00)
{
	var_01 = level.placeableconfigs[param_00];
	self.var_2E6 = self.placementorigin;
	self.var_41 = self.carriedobj.var_41;
	self playsoundasmaster(var_01.placedsfx);
	lib_A7CC::showplacedmodel(param_00);
	...
	if(isdefined(var_01.headiconheight))
	{
		if(level.teambased)
		{
			maps\mp\_entityheadicons::setteamheadicon(self.inliveplayerkillstreak,(0,0,var_01.headiconheight));
		}
		else
		{
	...
	thread lib_A7CC::handledamage(param_00);
	thread lib_A7CC::handledeath(param_00);
	self makeusable();
	...
	thread lib_A7CC::watchplayerconnected();
	self notify("placed");
	self.carriedobj delete();
...
//Function Number: 8
lib_A7CC::oncarried(param_00,param_01)
{
	var_02 = level.placeableconfigs[param_00];
	self.carriedobj = param_01 lib_A7CC::createcarriedobject(param_00);
	self.isplaced = undefined;
	self.carriedby = param_01;
	param_01.iscarrying = 1;
	lib_A7CC::deactivate(param_00);
	lib_A7CC::hideplacedmodel(param_00);
	if(isdefined(var_02.oncarrieddelegate))
	{
		self [[ var_02.oncarrieddelegate ]](param_00);
	}

	thread lib_A7CC::updateplacement(param_00,param_01);
	thread lib_A7CC::oncarrierdeath(param_00,param_01);
	self notify("carried");
}

//Function Number: 9
lib_A7CC::updateplacement(param_00,param_01)
{
	...
	for(;;)
	{
		...
		var_05.var_2E6 = self.placementorigin + var_04;
		var_05.var_41 = var_06["angles"];
		self.canbeplaced = param_01 isonground() && var_06["result"] && abs(self.placementorigin[2] - param_01.var_2E6[2]) < var_03.placementheighttolerance;
...
//Function Number: 12
lib_A7CC::handledamage(param_00)
{
	self endon("carried");
	var_01 = level.placeableconfigs[param_00];
	maps\mp\gametypes\_damage::setentitydamagecallback(var_01.var_275,var_01.damagefeedback,::lib_A7CC::handledeathdamage,::lib_A7CC::modifydamage,1);
}

//Function Number: 13
lib_A7CC::modifydamage(param_00,param_01,param_02,param_03)
{
	var_04 = param_03;
	var_05 = self.config;
...
//Function Number: 14
lib_A7CC::handledeathdamage(param_00,param_01,param_02,param_03)
{
	var_04 = self.config;
...
//Function Number: 16
lib_A7CC::oncarrierdeath(param_00,param_01)
{
	self endon("placed");
	self endon("death");
	param_01 endon("disconnect");
	...
	if(self.canbeplaced)
	{
		thread lib_A7CC::onplaced(param_00);
		return;
	}

	lib_A7CC::oncancel(param_00);
}

//Function Number: 17
lib_A7CC::onkillstreakdisowned(param_00)
{
	self endon("death");
	level endon("game_ended");
	self.owner waittill("killstreak_disowned");
	lib_A7CC::cleanup(param_00);
}

//Function Number: 18
lib_A7CC::ongameended(param_00)
{
	self endon("death");
...
//Function Number: 20
lib_A7CC::watchplayerconnected()
{
	self endon("death");
	for(;;)
	{
		level waittill("connected",var_00);
		thread lib_A7CC::onplayerconnected(var_00);
	}
}

//Function Number: 21
lib_A7CC::onplayerconnected(param_00)
{
	self endon("death");
...
//Function Number: 27
lib_A7CC::createbombsquadmodel(param_00)
{
	var_01 = level.placeableconfigs[param_00];
...
//Function Number: 30
lib_A7CC::createcarriedobject(param_00)
{
	if(isdefined(self.iscarrying) && self.iscarrying)
	{
		return;
	...
	var_01 = spawnturret("misc_turret",self.var_2E6 + (0,0,25),"sentry_minigun_mp");
