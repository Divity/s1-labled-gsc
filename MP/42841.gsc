...
//Function Number: 1
lib_A759::watchtridroneusage()
{
	self endon("spawned_player");
	self endon("disconnect");
	self endon("death");
	self endon("faux_spawn");
	lib_A759::tridroneammoinit();
	for(;;)
	{
		...
		if(var_01 == "tri_drone_mp")
		{
			...
			thread lib_A759::tryusetridrone(var_00);
		}
	}
}
...
//Function Number: 4
lib_A759::tryusetridrone(param_00)
{
	...
	if(self.tridroneammo > 0)
	{
		thread lib_A759::launchtridrone(param_00);
		self.tridroneammo--;
...
//Function Number: 5
lib_A759::launchtridrone(param_00)
{
	var_01 = lib_A759::spawnmine(param_00);
	self.tridronedeployed = common_scripts\utility::array_add(self.tridronedeployed,var_01);
	thread lib_A759::monitorplayerdeath(var_01);
}

//Function Number: 6
lib_A759::activategroupedtridrones(param_00)
{
	self endon("death");
...
lib_A759::spawnmine(param_00)
{
	...
	var_09.stunned = 0;
	var_09.fx_origin = "tri_drone_mp";
	param_00 delete();
	level.mines[level.mines.size] = var_09;
	var_09 thread lib_A759::createbombsquadmodel(level.tridronesettings.dronebombsquadmesh,"tag_origin",self);
	var_09 thread lib_A759::minebeacon();
	var_09 thread lib_A759::settridroneteamheadicon(self.inliveplayerkillstreak);
	var_09 thread lib_A759::minedamagemonitor();
	var_09 thread lib_A759::mineproximitytrigger(self);
	var_09 thread lib_A759::mineselfdestruct();
	var_09 thread lib_A759::deletemineonteamswitch(self);
	var_09 thread lib_A759::handleemp(self,"apm_mine");
	return var_09;
}

//Function Number: 10
lib_A759::showdebugradius()
{
...
//Function Number: 11
lib_A759::showammocount()
{
	self endon("disconnect");
	for(;;)
	{
		if("tri_drone_mp" == self getoffhandprimaryclass())
		{
...
//Function Number: 12
lib_A759::createbombsquadmodel(param_00,param_01,param_02)
{
...
lib_A759::minebeacon()
{
	...
	thread lib_A759::minebeaconteamupdater(var_00);
	self waittill("death");
...
//Function Number: 14
lib_A759::minebeaconteamupdater(param_00,param_01)
{
	self endon("death");
...
//Function Number: 15
lib_A759::settridroneteamheadicon(param_00)
{
	self endon("death");
	wait(0.05);
	if(level.teambased)
...
lib_A759::mineexplode(param_00)
{
	...
	self hide();
	self entityradiusdamage(self.var_2E6,192,60,60,param_00,"MOD_EXPLOSIVE","bouncingbetty_mp");
	foreach(var_03 in level.var_328)
	{
		var_04 = distance(self.var_2E6,var_03.var_2E6);
		if(var_04 < 192)
		{
	...
	thread lib_A759::apm_mine_deletekillcament();
	self notify("death");
...
//Function Number: 18
lib_A759::apm_mine_deletekillcament()
{
	wait(3);
...
lib_A759::equipmentwatchuse()
{
	...
	lib_A759::equipmentenableuse(var_00);
	for(;;)
	{
		self.pickuptrigger waittill("trigger",var_00);
		var_00 method_82F4("scavenger_pack_pickup");
		var_00.tridroneammo++;
...
//Function Number: 20
lib_A759::equipmentenableuse(param_00)
{
...
//Function Number: 21
lib_A759::equipmentdisableuse(param_00)
{
...
//Function Number: 22
lib_A759::mineproximitytrigger(param_00)
{
	self endon("mine_destroyed");
	self endon("mine_selfdestruct");
	self endon("death");
	wait(2);
	self.pickuptrigger = spawn("script_origin",self.var_2E6);
	self.pickuptrigger.owner = param_00;
	thread lib_A759::equipmentwatchuse();
	var_01 = spawn("trigger_radius",self.var_2E6 + (0,0,-96),0,192,192);
	var_01.owner = self;
	self.trigger = var_01;
	thread lib_A759::minedeletetrigger(var_01);
	var_02 = undefined;
	for(;;)
	{
	...
	thread lib_A759::mineexplode();
}

//Function Number: 23
lib_A759::playspinnerfx()
{
	self endon("death");
	var_00 = gettime() + 1000;
...
//Function Number: 24
lib_A759::minedeletetrigger(param_00)
{
...
lib_A759::mineselfdestruct()
{
	...
	playfx(level._effect["equipment_explode"],self.var_2E6);
	self delete();
}

//Function Number: 26
lib_A759::deletemineonteamswitch(param_00)
{
...
//Function Number: 27
lib_A759::handleemp(param_00,param_01)
{
	self endon("death");
