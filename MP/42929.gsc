...
lib_A7B1::init()
{
	...
	level thread lib_A7B1::onplayerconnect();
}

//Function Number: 2
lib_A7B1::onplayerconnect()
{
	for(;;)
	{
		level waittill("connected",var_00);
		var_00 thread lib_A7B1::onjoinedteam();
		var_00 thread lib_A7B1::onjoinedspectators();
		var_00 thread lib_A7B1::onspectatingclient();
	}
}

//Function Number: 3
lib_A7B1::onjoinedteam()
{
	self endon("disconnect");
	for(;;)
	{
		self waittill("joined_team");
		lib_A7B1::setspectatepermissions();
	}
}

//Function Number: 4
lib_A7B1::onjoinedspectators()
{
	self endon("disconnect");
	for(;;)
	{
		...
		lib_A7B1::setspectatepermissions();
		if(!maps\mp\_utility::invirtuallobby() && self ismlgspectator() || isdefined(self.connectedpostgame["mlgSpectator"]) && self.connectedpostgame["mlgSpectator"])
		{
	...
	if(self ismlgspectator())
	{
		for(;;)
		{
			...
			if(isdefined(var_01))
			{
				...
				{
					self method_8539(var_01.primary);
				}

				if(isdefined(var_01.secondary))
				{
					self method_8539(var_01.secondary);
				}
			}
		}
	}
...
lib_A7B1::onspectatingclient()
{
	...
	for(;;)
	{
		...
		if(isdefined(var_00))
		{
			...
			if(self ismlgspectator())
			{
				lib_A7B1::updatespectatedloadout(var_00);
			}
		}
	}
}
...
//Function Number: 7
lib_A7B1::updatespectatesettings()
{
	level endon("game_ended");
	for(var_00 = 0;var_00 < level.var_328.size;var_00++)
	{
		level.var_328[var_00] lib_A7B1::setspectatepermissions();
	}
}
...
//Function Number: 9
lib_A7B1::updatespectatedloadoutweapon(param_00,param_01,param_02)
{
...
//Function Number: 10
lib_A7B1::updatespectatedloadout(param_00)
{
	var_01 = param_00.spectatorviewloadout;
	lib_A7B1::updatespectatedloadoutweapon("ui_mlg_loadout_primary_",var_01.primary,[var_01.primaryattachment,var_01.primaryattachment2,var_01.primaryattachment3]);
	lib_A7B1::updatespectatedloadoutweapon("ui_mlg_loadout_secondary_",var_01.secondary,[var_01.secondaryattachment,var_01.secondaryattachment2]);
