...
//Function Number: 2
lib_A738::exo_knife_think()
{
	thread lib_A738::exo_knife_throw_watch();
}

//Function Number: 3
lib_A738::exo_knife_throw_watch()
{
	self endon("death");
	...
	for(;;)
	{
		...
		var_00.manuallydetonatefunc = ::lib_A738::exo_knife_manually_detonate;
		var_00.flying = 1;
		...
		var_00.owner = self;
		thread lib_A738::exo_knife_enable_detonate(var_00);
		var_00 thread lib_A738::exo_knife_touch_watch();
		var_00 thread lib_A738::exo_knife_stuck_watch();
		var_00 thread lib_A738::exo_knife_recall_watch();
		var_00 thread lib_A738::exo_knife_emp_watch();
	}
}

//Function Number: 4
lib_A738::exo_knife_emp_watch()
{
	self endon("death");
	for(;;)
	{
		...
		{
			thread lib_A738::exo_knife_delete();
		}
	}
}

//Function Number: 5
lib_A738::exo_knife_enable_detonate(param_00)
{
	self endon("disconnect");
...
//Function Number: 6
lib_A738::exo_knife_passed_target()
{
	self endon("death");
	self.owner endon("disconnect");
	self waittill("missile_passed_target");
	lib_A738::exo_knife_restock();
}

//Function Number: 7
lib_A738::exo_knife_touch_watch()
{
...
lib_A738::exo_knife_restock()
{
	...
	lib_A738::exo_knife_delete();
}

//Function Number: 9
lib_A738::exo_knife_stuck_watch()
{
	self endon("death");
	for(;;)
	{
		...
		if(isdefined(self.owner) && isdefined(var_00) && (isdefined(level.var_511A) && level.var_511A && var_00.model == "animal_dobernan") || maps\mp\_utility::isgameparticipant(var_00) && !var_01)
		{
			...
			self.owner thread lib_A738::exo_knife_recall(var_02);
			continue;
		}
...
//Function Number: 11
lib_A738::exo_knife_recall(param_00)
{
	self endon("death");
	self endon("disconnect");
...
//Function Number: 12
lib_A738::exo_knife_manually_detonate(param_00)
{
	if(isdefined(param_00) && !param_00.recall)
	{
		lib_A738::exo_knife_recall();
	}
}

//Function Number: 13
lib_A738::exo_knife_recall_watch()
{
	self endon("death");
	if(!isdefined(self.owner))
	{
	...
	var_09 thread lib_A738::exo_knife_passed_target();
	lib_A738::exo_knife_delete();
}

//Function Number: 14
lib_A738::exo_knife_clean_up_attractor(param_00,param_01,param_02)
{
