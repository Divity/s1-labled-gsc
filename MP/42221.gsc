...
//Function Number: 1
lib_A4ED::getexploders(param_00)
{
	param_00 = param_00 + "";
...
//Function Number: 2
lib_A4ED::setup_individual_exploder(param_00)
{
	...
	if(lib_A4ED::exploder_model_starts_hidden(param_00))
	{
		param_00 hide();
		return;
	}

	if(lib_A4ED::exploder_model_is_damaged_model(param_00))
	{
		param_00 hide();
		param_00 notsolid();
	...
	if(lib_A4ED::exploder_model_is_chunk(param_00))
	{
		param_00 hide();
		param_00 notsolid();
		if(isdefined(param_00.spawnflags) && param_00.spawnflags & 1)
...
lib_A4ED::setupexploders()
{
	...
	foreach(var_04 in var_00)
	{
		...
		{
			lib_A4ED::setup_individual_exploder(var_04);
		}
	}
	...
	for(var_02 = 0;var_02 < var_07.size;var_02++)
	{
		...
		if(isdefined(var_07[var_02].script_exploder))
		{
			if(!isdefined(var_07[var_02].var_41))
			{
				var_07[var_02].var_41 = (0,0,0);
			}
	...
	var_08["exploder"] = 1;
	thread lib_A4ED::setup_flag_exploders();
	for(var_02 = 0;var_02 < var_06.size;var_02++)
	{
		...
		var_04.v = [];
		var_04.v["origin"] = var_09.var_2E6;
		var_04.v["angles"] = var_09.var_41;
...
lib_A4ED::setup_flag_exploders()
{
	...
	{
		thread lib_A4ED::exploder_flag_wait(var_07);
	}
}

//Function Number: 5
lib_A4ED::exploder_flag_wait(param_00)
{
...
lib_A4ED::show_exploder_models_proc(param_00)
{
	...
	if(isdefined(level.createfxexploders))
	{
		...
		if(isdefined(var_01))
		{
			...
			{
				if(!lib_A4ED::exploder_model_starts_hidden(var_03.model) && !lib_A4ED::exploder_model_is_damaged_model(var_03.model) && !lib_A4ED::exploder_model_is_chunk(var_03.model))
				{
					var_03.model show();
				}
	...
	while(var_03 < level.createfxent.size)
	{
		...
		{
			if(!lib_A4ED::exploder_model_starts_hidden(var_05.model) && !lib_A4ED::exploder_model_is_damaged_model(var_05.model) && !lib_A4ED::exploder_model_is_chunk(var_05.model))
			{
				var_05.model show();
			}
...
//Function Number: 10
lib_A4ED::stop_exploder_proc(param_00)
{
	param_00 = param_00 + "";
	if(isdefined(level.createfxexploders))
	{
...
//Function Number: 11
lib_A4ED::get_exploder_array_proc(param_00)
{
	param_00 = param_00 + "";
	var_01 = [];
	if(isdefined(level.createfxexploders))
...
//Function Number: 12
lib_A4ED::hide_exploder_models_proc(param_00)
{
	param_00 = param_00 + "";
	if(isdefined(level.createfxexploders))
	{
...
//Function Number: 13
lib_A4ED::delete_exploder_proc(param_00)
{
	param_00 = param_00 + "";
	if(isdefined(level.createfxexploders))
	{
...
//Function Number: 15
lib_A4ED::activate_individual_exploder_proc()
{
	if(isdefined(self.v["firefx"]))
	{
		thread lib_A4ED::fire_effect();
	}

	if(isdefined(self.v["fxid"]) && self.v["fxid"] != "No FX")
	{
		thread lib_A4ED::cannon_effect();
	}
	else if(isdefined(self.v["soundalias"]) && self.v["soundalias"] != "nil")
	{
		thread lib_A4ED::sound_effect();
	}

	if(isdefined(self.v["loopsound"]) && self.v["loopsound"] != "nil")
	{
		thread lib_A4ED::effect_loopsound();
	}

	if(isdefined(self.v["damage"]))
	{
		thread lib_A4ED::exploder_damage();
	}

	if(isdefined(self.v["earthquake"]))
	{
		thread lib_A4ED::exploder_earthquake();
	}

	if(isdefined(self.v["rumble"]))
	{
		thread lib_A4ED::exploder_rumble();
	}

	if(self.v["exploder_type"] == "exploder")
	{
		thread lib_A4ED::brush_show();
		return;
	}

	if(self.v["exploder_type"] == "exploderchunk" || self.v["exploder_type"] == "exploderchunk visible")
	{
		thread lib_A4ED::brush_throw();
		return;
	}

	thread lib_A4ED::brush_delete();
}

//Function Number: 16
lib_A4ED::brush_delete()
{
...
lib_A4ED::brush_throw()
{
	...
	if(var_08)
	{
		...
		{
			var_0A = var_01;
			var_0B = var_00.var_2E6;
		}
		else
		{
			var_0A = var_02.var_2E6;
			var_0B = var_0A.var_2E6 - var_01.var_2E6 * self.v["physics"];
		}
...
lib_A4ED::exploder_rumble()
{
	...
	lib_A4ED::exploder_delay();
	level.var_31D playrumbleonentity(self.v["rumble"]);
}
...
//Function Number: 22
lib_A4ED::sound_effect()
{
	lib_A4ED::effect_soundalias();
}
...
//Function Number: 27
lib_A4ED::cannon_effect()
{
	if(isdefined(self.v["repeat"]))
	{
		thread lib_A4ED::exploder_playsound();
...
//Function Number: 28
lib_A4ED::activate_exploder(param_00,param_01,param_02)
{
	param_00 = param_00 + "";
	...
	if(isdefined(level.createfxexploders) && !level.createfx_enabled)
	{
		...
		if(isdefined(var_04))
		{
			...
			{
				if(!var_06 lib_A4ED::check_exploder_platform())
				{
					continue;
				}
	...
	if(!lib_A4ED::shouldrunserversideeffects() && !var_03)
	{
		lib_A4ED::activate_clientside_exploder(param_00,param_01,param_02);
	}
}

//Function Number: 29
lib_A4ED::exploder(param_00,param_01,param_02)
{
	[[ level._fx.exploderfunction ]](param_00,param_01,param_02);
}

//Function Number: 30
lib_A4ED::kill_exploder(param_00)
{
	var_01 = lib_A4ED::getexploders(param_00);
	if(isdefined(var_01))
	{
		foreach(var_03 in var_01)
		{
...
//Function Number: 31
lib_A4ED::check_exploder_platform()
{
	var_00 = self;
...
lib_A4ED::activate_clientside_exploder(param_00,param_01,param_02)
{
	if(!lib_A4ED::is_valid_clientside_exploder_name(param_00))
	{
		return;
	}

	var_03 = int(param_00);
	function_0222(var_03,param_01,param_02);
}

//Function Number: 33
lib_A4ED::deactivate_clientside_exploder(param_00,param_01,param_02)
{
	if(!lib_A4ED::is_valid_clientside_exploder_name(param_00))
	{
		return;
	...
	var_03 = int(param_00);
	function_0292(var_03,param_01,param_02);
}

//Function Number: 34
lib_A4ED::is_valid_clientside_exploder_name(param_00)
{
...
//Function Number: 35
lib_A4ED::shouldrunserversideeffects()
{
	if(common_scripts\utility::issp())
	{
		return 1;
...
//Function Number: 36
lib_A4ED::exploder_before_load(param_00,param_01,param_02)
{
	waittillframeend;
	waittillframeend;
	lib_A4ED::activate_exploder(param_00,param_01,param_02);
...
//Function Number: 37
lib_A4ED::exploder_after_load(param_00,param_01,param_02)
{
	lib_A4ED::activate_exploder(param_00,param_01,param_02);
}