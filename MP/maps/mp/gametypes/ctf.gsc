 ...
 * Platform: PC
 * Function Count: 57
 * Decompile Time: 3629 ms
 * Timestamp: 4/20/2022 8:15:23 PM
*******************************************************************/
...
main()
{
	...
	level.var_A322 = maps\mp\_utility::getintproperty("scr_win_by_captures",0);
...
{
	waittillframeend;
	maps\mp\_utility::setextrascore1(self.connectedpostgame["defends"]);
}
...
ctf()
{
	...
	level.var_91E8[game["defenders"]] = createteamflag(game["defenders"],"allies");
	level.var_91E8[game["attackers"]] = createteamflag(game["attackers"],level.matchbonus["allies"]);
	level.capzones[game["defenders"]] = createcapzone(game["defenders"],"allies");
	level.capzones[game["attackers"]] = createcapzone(game["attackers"],level.matchbonus["allies"]);
	assignteamspawns();
...
{
	if(!isdefined(self.inliveplayerkillstreak) || !isdefined(level.alliesflagstatus))
	{
		return;
	}
	...
	var_02 = undefined;
	var_03 = undefined;
	if(self.inliveplayerkillstreak == "allies" || self.inliveplayerkillstreak == "spectator")
	{
...
createteamflag(param_00,param_01)
{
	...
	var_06 = spawn("trigger_radius",var_02.var_2E6,0,96,var_02.height);
	var_02 = var_06;
	var_04[0] setmodel(level.flagmodel[param_00]);
	var_04[0].var_63C4 = var_04[0] setcontents(0);
	var_07 = var_04[0].var_2E6 + (0,0,32);
	var_08 = var_04[0].var_2E6 + (0,0,-32);
	var_09 = bullettrace(var_07,var_08,0,undefined);
	var_04[0].var_2E6 = var_09["position"];
...
createcapzone(param_00,param_01)
{
	...
	var_05 maps\mp\gametypes\_gameobjects::setkeyobject(level.var_91E8[maps\mp\_utility::getotherteam(param_00)]);
	var_05.onuse = ::onuse;
	var_05.oncantuse = ::oncantuse;
	var_06 = var_02.var_2E6 + (0,0,32);
	var_07 = var_02.var_2E6 + (0,0,-32);
...
onpickup(param_00)
{
	...
	param_00 method_82F4("mp_ctf_flag_pickup");
...
flagawayvo(param_00,param_01,param_02)
{
	...
	{
		wait(15);
		if(level.var_91E8[param_01] maps\mp\gametypes\_gameobjects::ishome())
		{
...
ondrop(param_00)
{
	...
	var_02 = level.matchbonus[var_01];
...
returnaftertime()
{
	...
	var_01 = level.matchbonus[var_00];
...
onuse(param_00)
{
	...
	level.var_91E8[var_02] returnflag();
	if(maps\mp\gametypes\_gameobjects::getownerteam() == "allies")
	{
		setomnvar("ui_mlg_game_mode_status_1",0);
	}
...
applyflagcarrierclass(param_00)
{
	...
	self.connectedpostgame["gamemodeLoadout"] = level.ctf_loadouts[self.inliveplayerkillstreak];
	self.gamemode_chosenclass = self.class;
	self.gamemode_carrierclass = 1;
	self.connectedpostgame["class"] = "gamemode";
	self.connectedpostgame["lastClass"] = "gamemode";
	self.class = "gamemode";
	self.lastclass = "gamemode";
	self notify("faux_spawn");
	maps\mp\gametypes\_class::giveandapplyloadout(self.inliveplayerkillstreak,"gamemode");
...
removeflagcarrierclass()
{
	...
	self.connectedpostgame["gamemodeLoadout"] = undefined;
	self notify("faux_spawn");
	maps\mp\gametypes\_class::giveandapplyloadout(self.inliveplayerkillstreak,self.class);
	if(self.loadoutkeepcurrentkillstreaks)
	{
		maps\mp\killstreaks\_killstreaks::updatekillstreaks(1);
	}
...
{
	if(isdefined(param_01) && isplayer(param_01) && param_01.connectedpostgame["team"] != self.connectedpostgame["team"])
	{
		...
		{
			var_0D = distance2dsquared(param_01.var_2E6,var_0C.curorigin);
			var_0E = distance2dsquared(self.var_2E6,var_0C.curorigin);
			if(var_0C.ownerteam == param_01.inliveplayerkillstreak)
			{
				if(var_0D < var_0A || var_0E < var_0A)
				{
					param_01 thread maps\mp\_events::defendobjectiveevent(self,param_09);
					param_01 maps\mp\_utility::setextrascore1(param_01.connectedpostgame["defends"]);
				}
			}

			if(var_0C.ownerteam == self.inliveplayerkillstreak)
			{
				if(var_0D < var_0A || var_0E < var_0A)
				{
...
attachflag(param_00)
{
	var_01 = level.matchbonus[self.connectedpostgame["team"]];
...
goliathattachflag()
{
	...
	{
		self.var_1BB6 thread flageffects();
	}
}
...
goliathdropflag()
{
	...
	if(isdefined(self.var_1BB6))
	{
		...
		self.var_1BB6 thread maps\mp\gametypes\_gameobjects::setdropped();
	}
}
...
get_nearest_capzone_team(param_00)
{
	...
	foreach(var_05 in level.capzones)
	{
		...
		if(var_01)
		{
			var_06 = getpathdist(param_00.var_2E6,var_05.curorigin,999999);
		}

		if(!isdefined(var_06) || var_06 == -1)
		{
			var_06 = distancesquared(var_05.curorigin,param_00.var_2E6);
		}
...
getnearestflagteam(param_00)
{
	...
	if(var_01)
	{
		var_03 = getpathdist(param_00.var_2E6,level.var_91E8["allies"].var_2E6,999999);
		var_04 = getpathdist(param_00.var_2E6,level.var_91E8["axis"].var_2E6,999999);
	}

	if(!isdefined(var_03) || var_03 == -1)
	{
		var_03 = distancesquared(level.var_91E8["allies"].var_2E6,param_00.var_2E6);
	}

	if(!isdefined(var_04) || var_04 == -1)
	{
		var_04 = distancesquared(level.var_91E8["axis"].var_2E6,param_00.var_2E6);
	}

	if(isdefined(param_00.var_39B))
	{
		if((game["switchedsides"] && param_00.var_39B == "axis_override") || !game["switchedsides"] && param_00.var_39B == "allies_override")
		{
			var_02 = "allies";
			param_00.friendlyflag = level.var_91E8["allies"];
			param_00.friendlyflagdist = var_03;
			param_00.enemyflagdist = var_04;
		}
		else if((game["switchedsides"] && param_00.var_39B == "allies_override") || !game["switchedsides"] && param_00.var_39B == "axis_override")
		{
			var_02 = "axis";
			param_00.friendlyflag = level.var_91E8["axis"];
	...
	if(!isdefined(var_02))
	{
		...
		param_00.friendlyflag = common_scripts\utility::ter_op(var_03 < var_04,level.var_91E8["allies"],level.var_91E8["axis"]);
...
flageffects()
{
	...
	friendlyenemyeffects(level.flaggroundfxid,var_00.var_2E6,anglestoup(var_00.var_41));
}
...
{
	var_04 = function_02C1(param_00,param_02,param_03);
...
{
	foreach(var_01 in level.var_91E8)
	{
		if(var_01.visuals.size)
		{
			var_02 = var_01.visuals[0].var_2E6 + (0,0,32);
			var_03 = var_01.visuals[0].var_2E6 + (0,0,-32);
