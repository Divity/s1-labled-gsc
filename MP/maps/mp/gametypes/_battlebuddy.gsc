 ...
 * Decompiled File: _battlebuddy - 42879.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 27
 * Decompile Time: 1747 ms
 * Timestamp: 4/20/2022 8:13:28 PM
*******************************************************************/
	...
	for(;;)
	{
		...
		if(var_00 == "battlebuddy_update")
		{
			...
			else if(hasbattlebuddy())
			{
				...
				{
					self.var_132A setupforrandomspawn();
				}
...
waitforbuddyspawntimer()
{
	...
	{
		self.var_132A displaybuddystatusmessage(&"MP_BUDDY_ERR_COMBAT");
	}
	else
	{
		self.var_132A displaybuddystatusmessage(&"MP_BUDDY_INCOMING");
	}
	...
	for(var_01 = checkbuddyspawn();var_01.status != 0;var_01 = checkbuddyspawn())
	{
		...
		else if(var_01.status == -2)
		{
			...
			self.var_132A displaybuddystatusmessage(&"MP_BUDDY_ERR_POINT");
		}
...
displaybuddyspawnsuccessful()
{
	...
	if(isdefined(self.var_132A))
	{
		self.var_132A displaybuddystatusmessage(&"MP_BUDDY_SPAWNED_ON_YOU");
		wait(1.5);
		self.var_132A maps\mp\_utility::clearlowermessage("waiting_info");
	}
}
...
{
	return param_00 needsbattlebuddy() && self.inliveplayerkillstreak == param_00.inliveplayerkillstreak;
}
