 ...
 * Platform: PC
 * Function Count: 35
 * Decompile Time: 2235 ms
 * Timestamp: 4/20/2022 8:15:33 PM
*******************************************************************/
...
infectedclass()
{
	...
	self.connectedpostgame["gamemodeLoadout"] = level.infect_loadouts[self.connectedpostgame["team"]];
...
onspawnplayer()
{
	...
	{
		self.connectedpostgame["gamemodeLoadout"] = level.infect_loadouts[self.connectedpostgame["team"]];
		maps\mp\gametypes\_class::giveloadout(self.inliveplayerkillstreak,self.class);
	...
	{
		self.connectedpostgame["gamemodeLoadout"] = level.infect_loadouts["axis_initial"];
		maps\mp\gametypes\_class::giveloadout(self.inliveplayerkillstreak,self.class);
	}
...
choosefirstinfected()
{
	...
	{
		if(maps\mp\_utility::matchmakinggame() && level.var_328.size > 1 && var_04 ishost())
		{
			var_02 = var_04;
			continue;
		}
...
prepareforclasschange()
{
	...
	while(!self isonground() && !self method_801A())
	{
		wait(0.05);
	}
...
setinitialtonormalinfected()
{
	...
	prepareforclasschange();
	self notify("faux_spawn");
	self.connectedpostgame["gamemodeLoadout"] = level.infect_loadouts["axis"];
	thread onspawnfinished();
	maps\mp\gametypes\_class::giveandapplyloadout(self.inliveplayerkillstreak,"gamemode");
	refillbattery();
}
...
onplayerkilled(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09)
{
	...
	if(self.inliveplayerkillstreak == "axis" && isplayer(param_01) && param_01.inliveplayerkillstreak == "allies" && maps\mp\_utility::ismeleemod(param_03))
	{
...
finalsurvivoruav(param_00)
{
	...
	{
		if(var_03.inliveplayerkillstreak == "axis")
		{
	...
	for(;;)
	{
		...
		wait(6);
		if(distance(var_05,param_00.var_2E6) < 200)
		{
			setteamradar("axis",1);
			var_01 = 1;
			foreach(var_03 in level.var_328)
			{
				var_03 method_82F4("recondrone_tag");
			}
		}
	}
}
	...
	if(isdefined(self.infect_isbeingchosen) || level.infect_chosefirstinfected)
	{
		if(level.infect_teamscores["axis"] && level.infect_teamscores["allies"])
		{
			...
			else if(var_00 == "axis" && level.infect_teamscores["axis"] == 1)
			{
				...
				{
					if(var_02 != self && var_02.inliveplayerkillstreak == "axis")
					{
