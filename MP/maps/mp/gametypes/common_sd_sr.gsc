 ...
 * Platform: PC
 * Function Count: 38
 * Decompile Time: 2401 ms
 * Timestamp: 4/20/2022 8:15:18 PM
*******************************************************************/
...
isplayeroutsideofanybombsite(param_00)
{
	if(isdefined(level.bombzones))
	{
		...
		{
			if(distancesquared(self.var_2E6,var_02.trigger.var_2E6) < 4096)
			{
				return 0;
			}
		}
...
{
	if(game["state"] == "postgame" && param_00.inliveplayerkillstreak == game["defenders"] || !level.bombplanted)
	{
...
onpickup(param_00)
{
	...
	{
		param_00 thread maps\mp\_matchdata::loggameevent("pickup",param_00.var_2E6);
	...
	if(isdefined(level.sd_loadout) && isdefined(level.sd_loadout[param_00.inliveplayerkillstreak]))
	{
	...
	{
		maps\mp\_utility::teamplayercardsplash("callout_bombtaken",param_00,param_00.inliveplayerkillstreak);
		maps\mp\_utility::leaderdialog("bomb_taken",param_00.connectedpostgame["team"]);
	}

	maps\mp\_utility::playsoundonplayers(game["bomb_recovered_sound"],game["attackers"]);
	param_00 method_82F4(game["bomb_grabbed_sound"]);
}
...
bombs()
{
	...
	for(var_07 = 0;var_07 < var_02.size;var_07++)
	{
		...
		var_08.bombdefusetrig.var_2E6 = var_08.bombdefusetrig.var_2E6 + (0,0,-10000);
		var_08.bombdefusetrig.label = var_09;
		var_08.bombdefusetrig method_8537(1);
	}
...
{
	if(!maps\mp\gametypes\_gameobjects::isfriendlyteam(param_00.connectedpostgame["team"]))
	{
		...
		param_00 playsoundasmaster("mp_bomb_plant");
...
{
	var_01 = common_scripts\utility::array_remove(level.var_328,param_00);
	if(var_01.size)
	{
...
{
	var_01 = spawn("script_origin",self.var_2E6);
	var_01.var_41 = self.var_41;
	var_01 rotateyaw(-45,0.05);
	wait(0.05);
	var_02 = self.var_2E6 + (0,0,5);
	var_03 = self.var_2E6 + anglestoforward(var_01.var_41) * 100 + (0,0,128);
...
onbeginuse(param_00)
{
	...
	if(!level.silentplant)
	{
		...
		param_00 playsoundasmaster("mp_snd_bomb_arming");
		level thread play_looping_beep_on_player(param_00);
	}
...
onenduse(param_00,param_01,param_02)
{
	...
	if(maps\mp\gametypes\_gameobjects::isfriendlyteam(param_01.connectedpostgame["team"]))
	{
...
bombplanted(param_00,param_01)
{
	...
	{
		level.sdbombmodel = spawn("script_model",param_01.var_2E6);
		level.sdbombmodel.var_41 = param_01.var_41;
	...
	var_03.var_2E6 = level.sdbombmodel.var_2E6;
	var_04 = [];
	...
	maps\mp\_utility::playsoundinspace("mp_snd_bomb_planted",level.sdbombmodel.var_2E6 + (0,0,1));
	bombtimerwait();
	...
	{
		var_06 = level.sdbombmodel.var_2E6;
		var_06 = var_06 + (0,0,10);
		level.sdbombmodel hide();
		stopfxontag(common_scripts\utility::getfx("bomb_light_planted"),level.sdbombmodel,"tag_flash");
		if(isdefined(param_01))
		{
			param_00.visuals[0] entityradiusdamage(var_06,512,300,20,param_01,"MOD_EXPLOSIVE","bomb_site_mp");
			param_01 thread maps\mp\_events::bombdetonateevent();
		}
		else
		{
			param_00.visuals[0] entityradiusdamage(var_06,512,300,20,undefined,"MOD_EXPLOSIVE","bomb_site_mp");
		}
...
givelastonteamwarning()
{
	...
	var_00 = maps\mp\_utility::getotherteam(self.connectedpostgame["team"]);
	level thread maps\mp\_utility::teamplayercardsplash("callout_lastteammemberalive",self,self.connectedpostgame["team"]);
...
applybombcarrierclass()
{
	...
	self.connectedpostgame["gamemodeLoadout"] = level.sd_loadout[self.inliveplayerkillstreak];
