 ...
 * Platform: PC
 * Function Count: 80
 * Decompile Time: 5556 ms
 * Timestamp: 4/20/2022 8:15:41 PM
*******************************************************************/
...
main()
{
	...
	level.idflags_stun = ::minion_damage;
...
onstartgametype()
{
	...
	if(!isdefined(level.momentum_multiplier_max))
	{
		level.momentum_multiplier_max = 60;
	}
...
create_active_zone()
{
	...
	var_01 = spawn("trigger_radius",(0,0,0),0,level.zone_radius,level.momentum_multiplier_max);
...
zone_flag_effect()
{
	...
	self.flagfx = function_02C1(level.flagfxid,var_00,"tag_fx_flag");
	function_0155(self.flagfx,1);
...
zone_boarder_effect()
{
	...
	var_00 = level.twar_use_obj.visuals[0];
	self.boarderfx = spawnfx(level.boarderfxid,var_00.var_2E6,anglestoup(var_00.var_41));
	function_0155(self.boarderfx,1);
...
update_flag_outline()
{
	for(;;)
	{
		...
		{
			if((var_04 == "allies" || var_04 == "axis") && var_09.inliveplayerkillstreak == "allies" || var_09.inliveplayerkillstreak == "axis")
			{
				if(var_04 == var_09.inliveplayerkillstreak)
				{
...
givezonecapturexp(param_00)
{
	...
	{
		var_01 = param_00[var_02[var_03]].var_31D;
		if(isdefined(var_01.owner))
		{
			var_01 = var_01.owner;
		}
...
onupdateuserate()
{
	...
	{
		var_06 = var_05.var_31D;
		if(!isdefined(var_06))
		{
			continue;
		}

		if(var_06.connectedpostgame["team"] != var_03)
		{
			continue;
		}
	...
	foreach(var_05 in self.touchlist[self.claimteam])
	{
		...
		if(var_06.connectedpostgame["team"] != self.claimteam)
		{
			continue;
		}
...
set_contested_zone(param_00)
{
	...
	level.twar_use_obj maps\mp\gametypes\_gameobjects::move_use_object(param_00.var_2E6,(0,0,100));
	foreach(var_02 in level.twar_zones)
	{
		if(var_02 != param_00)
		{
...
update_lua_hud()
{
	for(;;)
	{
		...
		if(var_05 != "none")
		{
			var_07 = level.twar_use_obj.curprogress / level.twar_use_obj.var_9C11;
		}
...
{
	return level.twar_team_multiplier[param_00] == level.var_5D5A;
}
...
clear_maxed_momentum(param_00)
{
	...
	set_momentum(param_00,0);
	set_momentum_multiplier(param_00,level.var_5D5A - 1);
...
add_momentum(param_00,param_01,param_02)
{
	...
	if(!var_05)
	{
		while(var_03 >= 1 && var_04 < level.var_5D5A)
		{
			var_03 = var_03 - 1;
			var_04++;
			var_05 = 1;
...
onnormaldeath(param_00,param_01,param_02)
{
	...
	{
		add_kill_enemy_momentum(param_01.inliveplayerkillstreak);
	}

	if(isdefined(param_00.inliveplayerkillstreak))
	{
		add_kill_friendly_momentum(param_00.inliveplayerkillstreak);
	}
}
...
get_start_spawn_centers(param_00)
{
	...
	if(param_00)
	{
		...
		foreach(var_05, var_08 in var_01)
		{
			...
			{
				var_11 = getpathdist(var_08,var_0D[var_10].var_2E6,99999,1);
				if(var_11 > 0)
				{
					var_0F = 1;
					break;
			...
			if(!var_0F)
			{
				var_01[var_05] = var_03[var_05][0].var_2E6;
			}
		}
	}
...
find_zones()
{
	...
	{
		var_06 = twar_zone(var_03,var_05.var_2E6,var_05.var_41,color_from_index(var_03));
...
get_zone_origins()
{
	...
	foreach(var_03 in var_01)
	{
		...
		var_0D = spawnstruct();
		var_0D.var_2E6 = var_03.var_2E6;
		if(isdefined(var_0C))
		{
			var_0D.var_41 = var_0C;
		}
		else
		{
			var_0D.var_41 = var_03.script_angles;
		}
...
get_zone_origins_auto()
{
	...
	foreach(var_0F in var_02)
	{
		...
		{
			var_10[var_12] = getpathdist(var_00[var_12],var_0F.var_2E6,99999,var_06);
		}
	...
	foreach(var_09, var_19 in var_0D)
	{
		...
		var_1D = spawnstruct();
		var_1D.var_2E6 = var_19[var_1C].var_2E6;
		var_16[var_09] = var_1D;
		var_17 = var_19[var_1C].var_2E6;
	}
...
twar_zone(param_00,param_01,param_02,param_03)
{
	...
	var_04.var_2E6 = var_07["position"];
	var_04.owner = "none";
	var_04.index = param_00;
	var_04.var_41 = param_02;
	var_04.debug_color = param_03;
	var_04.projector = spawn_flag_projector(var_04.var_2E6);
	return var_04;
}
...
draw_spawn_until_notify(param_00,param_01,param_02)
{
	...
	level thread draw_line_until_notify(param_00.var_2E6,param_00.var_2E6 + var_03 * 50,param_01,param_02);
...
{
	if(isdefined(level.twar_zones[param_00].var_41))
	{
		return anglestoforward(level.twar_zones[param_00].var_41);
	}
...
assign_spawns_version_3()
{
	...
	foreach(var_0E, var_03 in level.twar_zones)
	{
		...
		{
			var_07 = vectornormalize(var_03.var_2E6 - var_06.var_2E6);
...
getnearestspawns(param_00,param_01,param_02,param_03)
{
	...
	foreach(var_07 in var_05)
	{
		...
		if(var_04)
		{
			var_07.dist = getpathdist(var_07.var_2E6,param_00.var_2E6,999999);
		}

		if(var_07.dist == -1)
		{
			var_07.dist = distance(param_00.var_2E6,var_07.var_2E6);
		}
	}
...
getnearestzonepoint(param_00)
{
	...
	foreach(var_05 in level.twar_zones)
	{
		...
		if(var_01)
		{
			var_06 = getpathdist(param_00.var_2E6,var_05.var_2E6,999999);
		}

		if(!isdefined(var_06) || var_06 == -1)
		{
			var_06 = distance(var_05.var_2E6,param_00.var_2E6);
		}
...
update_minion_hud_outlines()
{
	...
	foreach(var_07 in level.agentarray)
	{
		if(var_07 is_minion())
		{
			...
			{
				var_07 hudoutlinedisableforclients(level.var_328);
			}
			...
			if(var_02)
			{
				if(var_00.size > 0)
				{
					var_07 hudoutlineenableforclients(var_00,common_scripts\utility::ter_op(var_07.inliveplayerkillstreak == "allies",2,3),1);
				}

				if(var_01.size > 0)
				{
					var_07 hudoutlineenableforclients(var_01,common_scripts\utility::ter_op(var_07.inliveplayerkillstreak == "axis",2,3),1);
				}
			}
		}
	}
...
minion_ai()
{
	...
	for(;;)
	{
		...
		{
			var_00 = level.twar_use_obj.trigger.var_2E6;
...
{
	if(isplayer(param_01) && is_minion() && param_01.inliveplayerkillstreak != self.inliveplayerkillstreak)
	{
		...
		if(isdefined(param_01))
		{
			add_kill_enemy_minion_momentum(param_01.inliveplayerkillstreak);
		}

		if(isdefined(self.inliveplayerkillstreak))
		{
			add_kill_friendly_minion_momentum(self.inliveplayerkillstreak);
		}
	}
}
