 ...
 * Platform: PC
 * Function Count: 39
 * Decompile Time: 2365 ms
 * Timestamp: 4/20/2022 8:15:26 PM
*******************************************************************/
		...
		foreach(var_01 in level.var_328)
		{
			...
			foreach(var_03 in level.domflags)
			{
				if(var_01 istouching(var_03.levelflag))
				{
					...
					{
						if(var_01.inliveplayerkillstreak == "allies")
						{
							var_01.objective = 2;
						}
						else
					...
					{
						if(var_01.inliveplayerkillstreak == "axis")
						{
							var_01.objective = 6;
						}
						else
...
getprefereddompoints(param_00,param_01)
{
	...
	if(param_00.size == level.domflags.size)
	{
		...
		var_02[var_05.useobj.var_2CFF] = 1;
		return var_02;
	}

	if(var_02.size == 1 && var_03.size == 2)
	{
		var_06 = maps\mp\_utility::getotherteam(self.inliveplayerkillstreak);
		var_07 = maps\mp\gametypes\_gamescore::_getteamscore(var_06) - maps\mp\gametypes\_gamescore::_getteamscore(self.inliveplayerkillstreak);
		if(var_07 > 25)
		{
	...
	else
	{
		...
		var_05[var_06.useobj.var_2CFF] = 1;
		return var_05;
	}
...
domflags()
{
	...
	for(var_05 = 0;var_05 < level.flags.size;var_05++)
	{
		...
		{
			var_07[0] = spawn("script_model",var_06.var_2E6);
			var_07[0].var_41 = var_06.var_41;
		}
		...
		var_0A = var_07[0].var_2E6 + (0,0,32);
		var_0B = var_07[0].var_2E6 + (0,0,-32);
		...
		var_07[0].var_2E6 = var_0C["position"];
	...
	level.startpos["allies"] = var_0F[0].var_2E6;
	level.startpos["axis"] = var_0E[0].var_2E6;
	level.bestspawnflag = [];
...
getunownedflagneareststart(param_00,param_01,param_02)
{
	...
	for(var_05 = 0;var_05 < level.flags.size;var_05++)
	{
		...
		var_07 = distancesquared(var_06.var_2E6,level.startpos[param_00]);
...
onbeginuse(param_00)
{
	...
	if(var_01 == "neutral")
	{
		statusdialog("securing" + self.label,param_00.inliveplayerkillstreak);
...
updatevisuals(param_00)
{
	...
	if(!function_0294(self.visuals[0]))
	{
		var_01 = self.visuals[0];
		friendlyenemyeffects(var_01.var_2E6,anglestoup(var_01.var_41));
	}
}
...
giveflagcapturexp(param_00,param_01)
{
	...
	{
		var_02 = param_00[var_03[var_04]].var_31D;
		if(isdefined(var_02.owner))
		{
			var_02 = var_02.owner;
		}
...
updatedomscores()
{
	...
	while(!level.gameended)
	{
		...
		if(var_00.size)
		{
			...
			if(var_00.size == 3 && var_02 != "none" && level.var_328.size > 5)
			{
				...
				if(level.threecaptime[var_02]["time"] > 60000 && level.threecaptime["axis"]["awarded"] == 0)
				{
					...
					{
						if(var_08.inliveplayerkillstreak != var_02)
						{
							continue;
						}
...
onplayerkilled(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07,param_08,param_09)
{
	...
	foreach(var_0D in param_01.touchtriggers)
	{
		...
		if(param_01.inliveplayerkillstreak != var_0E)
		{
	...
	{
		var_0E = var_0D.useobj.ownerteam;
		var_12 = distancesquared(var_0D.var_2E6,var_0B.var_2E6);
		var_13 = distancesquared(var_0D.var_2E6,param_01.var_2E6);
		if(var_0E == param_01.inliveplayerkillstreak)
		{
		...
		if(var_0E == var_0B.inliveplayerkillstreak)
		{
...
getnearestflagpoint(param_00)
{
	...
	foreach(var_05 in level.domflags)
	{
		...
		if(var_01)
		{
			var_06 = getpathdist(param_00.var_2E6,var_05.levelflag.var_2E6,999999);
		}

		if(!isdefined(var_06) || var_06 == -1)
		{
			var_06 = distancesquared(var_05.levelflag.var_2E6,param_00.var_2E6);
		}
