 ...
 * Platform: PC
 * Function Count: 96
 * Decompile Time: 7236 ms
 * Timestamp: 4/20/2022 8:15:15 PM
*******************************************************************/
...
initspawns()
{
	...
	level.var_8923 = "mp_ball_spawn";
	if(!getspawnarray(level.var_8923).size)
	{
		level.var_8923 = "mp_dom_spawn";
	}
	...
	var_00 = level.ball_goals[game["attackers"]].var_2E6;
	var_01 = 0;
	var_02 = maps\mp\gametypes\_spawnlogic::getspawnpointarray(level.allies_start_spawn_name);
	foreach(var_04 in var_02)
	{
		var_01 = var_01 + distance(var_04.var_2E6,var_00);
	}
	...
	foreach(var_04 in var_02)
	{
		var_06 = var_06 + distance(var_04.var_2E6,var_00);
	}
...
ball_init_map_min_max()
{
	...
	if(var_00.size > 0)
	{
		...
		{
			level.ball_mins = maps\mp\gametypes\_spawnlogic::expandmins(level.ball_mins,var_02.var_2E6);
			level.ball_maxs = maps\mp\gametypes\_spawnlogic::expandmaxs(level.ball_maxs,var_02.var_2E6);
		}
...
ball_goal_useobject()
{
	...
	{
		var_01.trigger = spawn("trigger_radius",var_01.var_2E6 - (0,0,var_01.radius),0,var_01.radius,var_01.radius * 2);
		var_01.useobject = maps\mp\gametypes\_gameobjects::createuseobject(var_02,var_01.trigger,[],(0,0,var_01.radius * 2.1));
		var_01.useobject.var_1C1 = var_01;
		...
		var_01.killcament = spawn("script_model",var_01.var_2E6);
...
ball_assign_team_spawns()
{
	var_00 = maps\mp\gametypes\_spawnlogic::getspawnpointarray(level.var_8923);
	...
	foreach(var_04 in var_00)
	{
		...
		var_05 = ball_get_path_dist(var_01,var_04.var_2E6);
		var_06 = ball_get_path_dist(var_02,var_04.var_2E6);
		if(isdefined(var_04.var_39B))
		{
			if((game["switchedsides"] && var_04.var_39B == "axis_override") || !game["switchedsides"] && var_04.var_39B == "allies_override")
			{
			...
			else if((game["switchedsides"] && var_04.var_39B == "allies_override") || !game["switchedsides"] && var_04.var_39B == "axis_override")
			{
...
ball_goal_fx()
{
	...
	{
		var_01.score_fx["friendly"] = spawnfx(common_scripts\utility::getfx("ball_goal_activated_friendly"),var_01.var_2E6,(1,0,0));
		var_01.score_fx["enemy"] = spawnfx(common_scripts\utility::getfx("ball_goal_activated_enemy"),var_01.var_2E6,(1,0,0));
	}
...
ball_spawn(param_00)
{
	...
	var_02 = spawn("script_model",var_01.var_2E6);
	...
	{
		var_04 = spawn("trigger_radius",var_02.var_2E6 - (0,0,var_03 / 2),0,var_03,var_03);
	}
...
ball_pass_or_shoot()
{
	...
	self.var_1BB6 waittill("dropped");
}
...
ball_pass_watch()
{
	...
	if(isdefined(self.var_1BB6))
	{
		...
		var_02 = self.pass_target.var_2E6;
		wait(0.15);
		if(isdefined(self.pass_target))
		{
			var_01 = self.pass_target;
		...
		self.var_1BB6 thread ball_pass_projectile(self,var_01,var_02);
	}
}
...
ball_pass_projectile(param_00,param_01,param_02)
{
	...
	{
		param_02 = param_01.var_2E6;
	}

	var_03 = (0,0,40);
	var_04 = vectornormalize(param_02 + var_03 - self.visuals[0].var_2E6);
	var_05 = var_04 * 1000;
	self.projectile = function_0071("gamemode_ball",self.visuals[0].var_2E6,var_05,30,param_00,1);
	if(isdefined(param_01))
	{
		self.projectile missilesettargetent(param_01);
	}
...
ball_create_killcam_ent()
{
	...
	self.killcament = spawn("script_model",self.visuals[0].var_2E6);
...
ball_on_projectile_death()
{
	...
	{
		if(var_00.var_2E6 != var_00.baseorigin + (0,0,4000))
		{
...
ball_shoot_watch()
{
	...
	if(isdefined(self.var_1BB6))
	{
		...
		thread ball_pass_or_throw_active();
		wait(0.25);
		self playsoundasmaster("mp_ul_ball_throw");
		thread ball_check_pass_kill_pickup(self.var_1BB6);
		self.var_1BB6 ball_create_killcam_ent();
		self.var_1BB6 thread ball_physics_launch_drop(var_04 * var_01,self);
	}
}
...
ball_physics_launch(param_00,param_01)
{
	...
	ball_fx_start();
	var_02 physicslaunchserver(var_02.var_2E6,param_00);
...
ball_return_home()
{
	self.in_goal = 0;
	var_00 = self.visuals[0];
	playsoundatpos(var_00.var_2E6,"uplink_out_of_bounds");
	playfx(common_scripts\utility::getfx("ball_teleport"),var_00.var_2E6);
...
ball_physics_out_of_level()
{
	...
	for(;;)
	{
		...
		{
			if(var_00.var_2E6[var_03] > level.ball_maxs[var_03] + var_01[var_03])
			{
				ball_return_home();
				return;
			}

			if(var_00.var_2E6[var_03] < level.ball_mins[var_03] - var_02[var_03])
			{
				ball_return_home();
				return;
			}
...
ball_physics_touch_cant_pickup_player(param_00)
{
	...
	for(;;)
	{
		...
		if(var_01.var_2E6 == var_01.baseorigin + (0,0,4000))
		{
			continue;
		}
...
ball_physics_fake_bounce()
{
	...
	var_00 method_84E1();
	var_00 physicslaunchserver(var_00.var_2E6,var_03 * var_02);
}
...
ball_touch_goal_watch(param_00)
{
	for(;;)
	{
		foreach(var_05, var_02 in level.ball_goals)
		{
			...
			{
				var_04 = line_interect_sphere(param_00.origin_prev,param_00.var_2E6,var_02.var_2E6,var_02.radius);
				if(var_04)
				{
					thread ball_touched_goal(var_02);
					return;
...
ball_carrier_touched_goal(param_00)
{
	...
	if(!isdefined(param_00) || !isdefined(param_00.var_1BB6))
	{
		return;
	}

	if(isdefined(param_00.var_1BB6.scorefrozenuntil) && param_00.var_1BB6.scorefrozenuntil > gettime())
	{
		return;
	}

	param_00.var_1BB6.scorefrozenuntil = gettime() + 10000;
	param_00 maps\mp\_events::touchdownevent(var_01);
	ball_check_assist(param_00,1);
	param_00 thread ball_update_extrascore0();
	var_02 = self.var_1C1.inliveplayerkillstreak;
	...
	{
		var_04 = self.var_1C1.killcament;
		var_05 = var_04 getentitynumber();
		var_06 = var_04.var_7D;
		if(!isdefined(var_06))
		{
			var_06 = 0;
		}
	...
	ball_play_score_fx(self.var_1C1);
	ball_score_sound(var_03);
	if(isdefined(param_00.var_83D3))
	{
		param_00.var_83D3.inuse = 0;
	}

	var_07 = param_00.var_1BB6;
	var_07.var_5582 = 1;
	var_07 ball_set_dropped(1);
	var_07 thread ball_score_event(self.var_1C1);
...
ball_touched_goal(param_00)
{
	...
	if(isdefined(self.var_5581))
	{
		self.var_5582 = 1;
		self.var_5581 maps\mp\_events::fieldgoalevent(var_01);
		ball_check_assist(self.var_5581,0);
		self.var_5581 thread ball_update_extrascore0();
	...
	if(isdefined(self.var_5581))
	{
		setomnvar("ui_mlg_game_mode_status_2",self.var_5581 getentitynumber());
	}
	else
	{
		setomnvar("ui_mlg_game_mode_status_2",-1);
...
ball_score_event(param_00)
{
	...
	var_03 = 1.2;
	var_04 = 1;
	playsoundatpos(param_00.var_2E6,"uplink_goal_point");
	var_05 = var_02 + var_04;
	var_06 = var_05 + var_03;
	var_01 moveto(param_00.var_2E6,var_02,0,var_02);
...
ball_play_local_team_sound(param_00,param_01,param_02)
{
	...
	{
		if(var_05.inliveplayerkillstreak == param_00)
		{
			var_05 method_82F4(param_01);
			continue;
		}

		if(var_05.inliveplayerkillstreak == var_03)
		{
			var_05 method_82F4(param_02);
		}
	}
}
...
ball_on_pickup(param_00)
{
	...
	var_04 = maps\mp\_utility::getotherteam(param_00.inliveplayerkillstreak);
	if(var_02)
	{
		if(self.var_5583 == param_00.inliveplayerkillstreak)
		{
			maps\mp\_utility::leaderdialog("pass_complete",var_03,"status");
			param_00.passtime = gettime();
			param_00.passplayer = self.var_5581;
		}
		else
		{
	...
	param_00 playsoundasmaster("mp_ul_ball_catch");
	ball_play_local_team_sound(var_03,"mp_obj_notify_pos_sml","mp_obj_notify_neg_sml");
	ball_fx_stop();
	self.var_5582 = 0;
	self.var_5581 = param_00;
	self.var_5583 = param_00.inliveplayerkillstreak;
	self.ownerteam = param_00.inliveplayerkillstreak;
	ball_waypoint_held();
	...
	if(self.carrier.inliveplayerkillstreak == "allies")
	{
		setomnvar("ui_mlg_game_mode_status_3",1);
	}
	else
	...
	param_00.var_473B = param_00 hasperk("specialty_sprintfire",1);
...
ball_check_pass_kill_pickup(param_00)
{
	...
	var_01 timer_cancel();
	if(!isdefined(param_00.carrier) || param_00.carrier.inliveplayerkillstreak == self.inliveplayerkillstreak)
	{
		return;
	}
...
player_update_pass_target(param_00)
{
	...
	for(;;)
	{
		...
		if(!self method_801A())
		{
			...
			{
				if(var_07.inliveplayerkillstreak != self.inliveplayerkillstreak)
				{
					continue;
				}
...
player_update_pass_target_hudoutline()
{
	...
	self hudoutlinedisableforclients(level.var_328);
	...
	var_03 = [];
	var_04 = [];
	var_05 = maps\mp\_utility::getotherteam(self.inliveplayerkillstreak);
	foreach(var_01 in level.var_328)
	{
		if(var_01 == self)
		{
		...
		if(var_01.inliveplayerkillstreak == var_05)
		{
...
player_set_pass_target(param_00)
{
	...
	if(!isdefined(self.pass_target) && !function_0294(self.pass_target) && !isdefined(param_00))
	{
		return;
	}
	...
	if(isdefined(param_00))
	{
		...
		foreach(var_04 in level.var_328)
		{
			if(var_04.inliveplayerkillstreak == self.inliveplayerkillstreak && var_04 != self && var_04 != param_00)
			{
...
player_clear_pass_target()
{
	...
	foreach(var_02 in level.var_328)
	{
		if(var_02.inliveplayerkillstreak == self.inliveplayerkillstreak && var_02 != self)
		{
...
ball_set_dropped(param_00)
{
	...
	self notify("dropped");
	var_01 = self.carrier;
	if(isdefined(var_01) && var_01.inliveplayerkillstreak != "spectator")
	{
	...
	{
		self.visuals[var_04].var_2E6 = var_02;
		self.visuals[var_04].var_41 = var_03;
		self.visuals[var_04] show();
		self.visuals[var_04] method_8568(1);
	}

	self.trigger.var_2E6 = var_02;
	ball_dont_interpolate();
...
ball_assign_start(param_00)
{
	...
	{
		var_02.baseorigin = param_00.var_2E6;
	}

	self.trigger.baseorigin = param_00.var_2E6;
...
ball_on_reset()
{
	...
	if(!self.var_5582 && isdefined(var_03) && isdefined(var_02))
	{
		maps\mp\_utility::leaderdialog("drone_reset",var_03,"status");
		maps\mp\_utility::leaderdialog("drone_reset",var_02,"status");
		if(isdefined(self.var_5581))
		{
			thread maps\mp\_utility::teamplayercardsplash("callout_ballreset",self.var_5581);
		}
	}
...
ball_create_team_goal(param_00)
{
	...
	if(isdefined(var_03))
	{
		...
		if(var_02)
		{
			var_03.var_2E6 = var_03.ground_origin + (0,0,90);
		}
	}
	else
	{
...
ball_add_start(param_00)
{
	var_01 = 30;
	var_02 = spawnstruct();
	var_02.var_2E6 = param_00;
	var_02 ball_find_ground();
	var_02.var_2E6 = var_02.ground_origin + (0,0,var_01);
	var_02.in_use = 0;
...
ball_goal_fx_for_player(param_00)
{
	...
	foreach(var_06, var_03 in level.ball_goals)
	{
		...
		var_05 = spawnfxforclient(common_scripts\utility::getfx(var_04),var_03.var_2E6,param_00,(1,0,0));
		function_0155(var_05,1);
