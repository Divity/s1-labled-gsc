 ...
 * Decompiled File: _vl_base - 42843.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 30
 * Decompile Time: 1109 ms
 * Timestamp: 4/20/2022 8:18:02 PM
*******************************************************************/
...
vl_main()
{
	...
	{
		level.var_117B[var_01] = spawnstruct();
		level.var_117B[var_01].var_9379 = 0;
		level.var_117B[var_01].num_lobby_idles = "";
	}
...
player_sticks_in_lefty_config()
{
	...
	{
		var_00 = self getplayerprofiledata("gpadSticksConfig");
...
player_update_right_stick_y(param_00)
{
	...
	if(player_sticks_in_lefty_config())
	{
		...
		param_00.fakeangle = angleclamp360(param_00.fakeangle + var_04);
		var_01 = param_00.fakeangle;
	}
	else
	{
		var_02 = self method_844D();
		var_01 = var_02[1];
		if(isdefined(param_00.fakeangle))
		{
...
prep_for_controls(param_00,param_01)
{
	...
	param_00.storedangley = param_00.var_41[1];
	param_00.mouserot = 0;
...
{
	return level.var_117B[param_00].var_9379 > 0;
}
...
all_avatars_scheduled_for_delete()
{
	...
	foreach(var_02, var_01 in level.var_117B)
	{
		if(!isdefined(level.is_cao_agent[var_02]))
		{
			continue;
		}

		if(var_01.var_9379 == 0)
		{
			return 0;
		}
	}
...
reuse_avatar(param_00)
{
	...
	{
		var_01 = level.is_cao_agent[var_03];
		level.is_cao_agent[var_03] = undefined;
		function_02D4(var_01,var_04,1);
		level.xuid2ownerid[var_04] = undefined;
	}

	level.xuid2ownerid[param_00] = 0;
	level.var_117B[0].var_9379 = 0;
	level.var_117B[0].num_lobby_idles = param_00;
	if(isdefined(var_01))
	{
		function_02D4(var_01,param_00);
		level.is_cao_agent[0] = var_01;
	}
}
...
add_avatar(param_00)
{
	...
	level.var_117B[var_01].num_lobby_idles = param_00;
	level.var_117B[var_01].var_9379 = 0;
	return var_01;
}
...
{
	var_00 = 0;
	foreach(var_03, var_02 in level.var_117B)
	{
		if(var_02.var_9379 > 0 && gettime() > var_02.var_9379)
		{
...
hide_non_owner_avatars()
{
	...
	if(level.camdirectionforward.var_5D32 != "prelobby")
	{
		level.camdirectionforward.camera.cut = 1;
	}
...
rightstickrotateavatar(param_00,param_01)
{
	...
	var_03 = angleclamp360(var_02 - param_00.storedrightsticky);
	var_04 = getdvarfloat("ui_mouse_char_rot",0);
	if(var_04 != 0)
	{
		param_00.mouserot = angleclamp360(param_00.mouserot + var_04);
		setdynamicdvar("ui_mouse_char_rot",0);
	}

	var_05 = maps\mp\_vl_camera::getmodifiedrotationangle(param_00,var_03,param_01);
	var_05 = var_05 * -1;
	var_06 = angleclamp360(param_00.storedangley + var_05 + param_00.mouserot);
	var_07 = (0,var_06,0);
	if(isagent(param_00))
	{
...
playerhastouchedstick(param_00)
{
	...
	var_02 = angleclamp360(var_01 - param_00.storedrightsticky);
	if(self.var_4748 == 0)
	{
		if(abs(var_02) >= 1)
		{
...
enter_vlobby(param_00)
{
	...
	param_00 setorigin(var_01.var_2E6);
	...
	{
		param_00 prep_for_controls(level.is_cao_agent[level.var_63BB],level.is_cao_agent[level.var_63BB].var_41);
	}
	...
	for(;;)
	{
		if(isdefined(level.in_firingrange))
		{
			...
			if(var_01 == 1 && !level.in_firingrange)
			{
				...
				while(var_07.size > 0)
				{
					var_08 = param_00 method_8511(var_07);
					if(var_08 == 1)
					{
						break;
					}
				...
				param_00 method_8481();
				maps\mp\_vl_firingrange::enter_firingrange(param_00);
				param_00 method_84D8("mp_no_foley",1);
			...
			{
				param_00 method_8482();
				param_00 firingrangecleanup();
				param_00 disable_player_controls();
				if(isdefined(param_00.alias))
				{
					param_00 switchtoweapon(param_00.alias);
				}

				param_00 notify("enter_lobby");
				enter_vlobby(param_00);
				param_00 method_84D7("mp_no_foley",1);
				setdvar("r_dof_physical_enable",1);
				setdvar("r_dof_physical_bokehEnable",1);
				setdvar("r_uiblurdstmode",3);
				setdvar("r_blurdstgaussianblurradius",1.5);
