 ...
 * Decompiled File: _assaultdrone_ai - 42937.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 24
 * Decompile Time: 1177 ms
 * Timestamp: 4/20/2022 8:15:54 PM
*******************************************************************/
...
assault_vehicle_ai_aerial_follow_path_outside(param_00)
{
	...
	for(var_03 = 0;var_03 < param_00.size;var_03++)
	{
		...
		assault_vehicle_ai_air_movement_func(var_04.var_2E6 + var_01);
		var_05 = 0;
...
assault_vehicle_ai_aerial_pathing_c4()
{
	for(;;)
	{
		...
		if(assault_vehicle_ai_enemy_exists_and_is_alive())
		{
			if(!assault_vehicle_ai_enemy_moved_air(var_00) || distance2dsquared(self.var_2E6,self.enemy_target.var_2E6) < squared(200))
			{
				var_03 = self.enemy_target.var_2E6 + (0,0,40);
				self veh_setgoalpos(var_03,1);
				while(assault_vehicle_ai_enemy_exists_and_is_alive() && distancesquared(var_03,self.var_2E6) > squared(24))
				{
					wait(0.05);
				}
...
assault_vehicle_ai_can_see_living_enemy()
{
	...
	if(gettime() > self.enemy_target_last_vis_time)
	{
		...
		if(isdefined(level.var_511A) && level.var_511A)
		{
			if(isdefined(self.enemy_target.ishordedrone) && self.enemy_target.ishordedrone)
			{
				self.enemy_target_visible = sighttracepassed(assault_vehicle_ai_get_camera_position(),self.enemy_target.var_2E6,0,self,self.enemy_target);
			}
			else
			{
				self.enemy_target_visible = sighttracepassed(assault_vehicle_ai_get_camera_position(),self.enemy_target.var_2E6 + (0,0,40),0,self,self.enemy_target);
			}
		}
		else
		{
