 ...
 * Decompiled File: streak_mp_bigben2.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 26
 * Decompile Time: 1253 ms
 * Timestamp: 4/20/2022 8:16:51 PM
*******************************************************************/
...
init()
{
	...
	level.rocket_start = var_00.var_2E6;
	level.rocket_start_angles = (270,0,0);
	level.rocket_end = var_01.var_2E6;
	level.camera_start = var_02.var_2E6;
	level.camera_end = var_03.var_2E6;
	...
	level.skylight_actual_rocket.var_2E6 = level.rocket_start;
	level.skylight_actual_rocket.var_41 = level.rocket_start_angles;
	level.skylight_camera_link.var_2E6 = level.camera_start;
...
tryuseskylight(param_00,param_01)
{
	...
	maps\mp\_matchdata::logkillstreakevent("dlc_streak4",self.var_2E6);
	return var_02;
}
...
skylight_monitor_fire()
{
	...
	self method_82F4("bb_missile_launch_pod_midair_exp");
	var_00 = self getplayerangles();
	var_01 = anglestoforward(var_00);
	var_02 = level.skylight_camera_link.var_2E6;
	...
	earthquake(0.4,1,level.skylight_camera_link.var_2E6,2000,self);
	wait(0.2);
	self notify("skylight_control_over");
	thread func_37B9(var_05);
}
...
skylight_cleanup_rocket()
{
	...
	wait(0.1);
	playfx(common_scripts\utility::getfx("mp_bigb_killstreak_rocket_explosion"),level.skylight_actual_rocket.var_2E6);
...
skylight_play_rumble()
{
	...
	wait(0.1);
	earthquake(0.4,3,level.skylight_camera_link.var_2E6,2000,self);
	wait(0.1);
...
skylight_play_rumble_on_all_players()
{
	foreach(var_01 in level.var_328)
	{
		if(var_01 != self)
		{
			...
			if(distancesquared(level.rocket_start,var_01.var_2E6) < 1000000)
			{
				var_01 playrumbleonentity("heavygun_fire");
				continue;
			}

			if(distancesquared(level.rocket_start,var_01.var_2E6) < 4000000)
			{
...
skylight_rocket_control()
{
	...
	level.skylight_fire_fx_ent = function_02C2(level.skylight_fire_fx,level.skylight_camera_link,"tag_player",self);
	function_0155(level.skylight_fire_fx_ent,1);
...
aud_skylight_launch_sfx()
{
	...
	self method_82F4("bb_missile_jet_pov_effit");
	common_scripts\utility::waittill_any("skylight_control_over","skylight_cancel");
	self stopsounds();
}
...
{
	wait(0.2);
	self playsoundasmaster("bb_missile_launch_pod_midair_exp_3d");
	self playsoundasmaster("bb_missile_low_3d");
	wait(0.2);
	self playsoundasmaster("bb_missile_jet_pov_effit_3d");
	common_scripts\utility::waittill_any("skylight_control_over","skylight_cancel");
	self stopsounds();
}
...
func_37B9(param_00)
{
	...
	var_02 = magicbullet("iw5_dlcgun12loot5_mp",level.skylight_actual_rocket.var_2E6,param_00,self);
	wait(0.1);
	if(randomint(100) <= level.skylight_fx_missile_chance && var_01 < level.skylight_fx_missile_max)
	{
		var_01++;
		var_03 = vectortoangles(level.skylight_actual_rocket.var_2E6 - param_00);
		var_04 = anglestoforward(var_03);
		playfx(common_scripts\utility::getfx("mp_bigb_killstreak_curvy_missile"),level.skylight_actual_rocket.var_2E6,var_04);
	}

	maps\mp\_audio::snd_play_in_space("paladin_cannon_snap",level.skylight_actual_rocket.var_2E6);
	for(var_05 = 1;var_05 < level.skylight_real_missile_number;var_05++)
	{
		...
		var_02 = magicbullet("iw5_dlcgun12loot5_mp",level.skylight_actual_rocket.var_2E6,param_00 + (var_06,var_07,0),self);
		wait(0.1);
		if(randomint(100) <= level.skylight_fx_missile_chance && var_01 < level.skylight_fx_missile_max)
		{
			var_01++;
			var_03 = vectortoangles(level.skylight_actual_rocket.var_2E6 - param_00 + (var_06,var_07,0));
			var_04 = anglestoforward(var_03);
			playfx(common_scripts\utility::getfx("mp_bigb_killstreak_curvy_missile"),level.skylight_actual_rocket.var_2E6,var_04);
		}
