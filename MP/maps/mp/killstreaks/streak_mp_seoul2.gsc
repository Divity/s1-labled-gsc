 ...
 * Decompiled File: streak_mp_seoul2.gsc
 * Game: Call of Duty: Advanced Warfare
 * Platform: PC
 * Function Count: 43
 * Decompile Time: 2126 ms
 * Timestamp: 4/20/2022 8:16:55 PM
*******************************************************************/
...
tryusempswarm(param_00,param_01)
{
	...
	maps\mp\_matchdata::logkillstreakevent("dlc_streak5",self.var_2E6);
	return 1;
}
...
setswarmplayer(param_00)
{
	...
	level.swarm_planemodel.var_31D = param_00;
...
setswarmvisionandlightsetpermap(param_00)
{
	...
	{
		self method_847A(level.swarmvisionset,0);
	}

	if(isdefined(level.swarmlightset))
	{
		self method_83C0(level.swarmlightset);
	}
...
removeswarmplayer(param_00,param_01)
{
	...
	waittillframeend;
	level.swarm_planemodel.var_31D = undefined;
	if(!param_01)
	{
		param_00 playerresetswarmomnvars();
		param_00 method_849C("swarm_fire","+attack");
		param_00 method_849C("swarm_fire","+attack_akimbo_accessible");
	...
	level.swarm_planemodel playsoundasmaster("paladin_orbit_return");
}
...
swarm_spawn()
{
	...
	level.swarm_planemodel.var_41 = (0,0,0);
	level.swarm_planemodel setmodel("vehicle_mp_seoul2_killstreak_osp_rig");
	level.swarm_planemodel.owner = self;
	level.swarm_planemodel common_scripts\utility::make_entity_sentient_mp(self.inliveplayerkillstreak);
	...
	level.swarm_planemodel.minimapicon method_8446(level.swarm_planemodel,"tag_origin",(0,0,0),(0,0,0));
	level.swarm_planemodel.fxent = level.swarm_planemodel common_scripts\utility::spawn_tag_origin();
	level.swarm_planemodel.fxent method_8446(level.swarm_planemodel,"tag_origin",(0,0,-800),(0,0,0));
...
spawnswarmturret(param_00,param_01,param_02)
{
	...
	var_03.var_41 = level.swarm_planemodel gettagangles(param_02);
...
firerocketswarmgun()
{
	...
	for(;;)
	{
		...
		{
			earthquake(0.3,1,level.swarm_planemodel.var_2E6,1000,self);
			...
			for(var_04 = 0;var_04 < 1;var_04++)
			{
				...
				var_08 show();
				var_08 method_8446(var_07,"tag_fx");
				var_09 = [];
				foreach(var_0B in level.var_328)
				{
					if(var_0B != self)
					{
						var_09[var_09.size] = function_02C2(common_scripts\utility::getfx("drone_swarm_trail"),var_08,"tag_origin",var_0B);
					}
				}

				var_09[var_09.size] = function_02C2(common_scripts\utility::getfx("drone_swarm_trail_inair"),var_08,"tag_origin",self);
				foreach(var_0E in var_09)
				{
					function_0155(var_0E,1);
					triggerfx(var_0E);
				}

				thread cleanupdronemissilefx(var_07,var_08,var_09);
				var_07.var_9CBC = level.swarm_planemodel;
				self method_82F4("paladin_missile_shot_2d");
				self playrumbleonentity("ac130_40mm_fire");
				var_07 missilesettargetent(level.swarm_targetent);
				var_07 missilesetflightmodedirect();
				wait(0.1);
			}
...
updateshootinglocation()
{
	...
	level.swarm_big_turret method_8508(level.swarm_targetent);
	for(;;)
	{
		...
		level.swarm_targetent.var_2E6 = var_03;
		wait 0.05;
	}
}
...
{
	var_00 = level.var_5983[2] + 3000;
	var_01 = 2000;
	var_02 = (0,randomint(360),0);
	level.swarm_planemodel.var_41 = var_02;
	level.swarm_planemodel.var_2E6 = level.swarm_planemodel.var_2E6 - vectornormalize(-1 * anglestoright(level.swarm_planemodel gettagangles("tag_origin"))) * var_01;
	level.swarm_planemodel.var_2E6 = level.swarm_planemodel.var_2E6 + (0,0,var_00);
	level.swarm_planemodel.destination2 = spawnstruct();
	level.swarm_planemodel.destination2.var_2E6 = level.swarm_planemodel.var_2E6;
	level.swarm_planemodel.destination2.var_41 = level.swarm_planemodel.var_41;
	level.swarm_planemodel.var_2E6 = level.swarm_planemodel.var_2E6 + (0,0,65000);
}
...
moveswarmtodestination(param_00)
{
	...
	if(param_00)
	{
		level.swarm_planemodel method_8446(level.swarmrig,"tag_player");
		thread rotateswarm(level.swarm_speed);
	}
	...
	if(isdefined(level.swarm_planemodel.owner))
	{
		...
		level.swarm_planemodel.closeflightsound method_8446(level.swarm_planemodel,"tag_origin",(0,0,0),(0,0,0));
	...
	level.swarm_planemodel.farflightsound method_8446(level.swarm_planemodel,"tag_origin",(0,0,0),(0,0,0));
...
playentrysounddelayed()
{
	...
	level endon("swarm_player_removed");
	wait(1);
	playsoundatpos(level.swarm_planemodel.destination2.var_2E6,"paladin_orbit_drop");
}
...
setupplayersduringstreak()
{
	...
	foreach(var_01 in level.var_328)
	{
		if(isdefined(var_01.inliveplayerkillstreak) && level.swarm_planemodel.owner.inliveplayerkillstreak == var_01.inliveplayerkillstreak)
		{
			continue;
		}
...
rotateswarm(param_00,param_01)
{
	...
	if(param_01 == "off")
	{
		...
		level.swarmrig rotateyaw(level.swarmrig.var_41[2] + var_02,var_03,0,var_03);
	}
}