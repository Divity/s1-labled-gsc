 ...
 * Platform: PC
 * Function Count: 111
 * Decompile Time: 6544 ms
 * Timestamp: 4/20/2022 8:13:10 PM
*******************************************************************/
...
main()
{
	...
	{
		maps\mp\bots\_bots_ks::func_167A();
	}
...
initbotmapextents()
{
	...
	if(var_00.size > 1)
	{
		level.bot_map_min_x = var_00[0].var_2E6[0];
		level.bot_map_max_x = var_00[0].var_2E6[0];
		level.bot_map_min_y = var_00[0].var_2E6[1];
		level.bot_map_max_y = var_00[0].var_2E6[1];
		level.bot_map_min_z = var_00[0].var_2E6[2];
		level.bot_map_max_z = var_00[0].var_2E6[2];
		for(var_01 = 1;var_01 < var_00.size;var_01++)
		{
			var_02 = var_00[var_01].var_2E6;
...
refresh_existing_bots()
{
	...
	foreach(var_01 in level.var_328)
	{
		if(isbot(var_01))
		{
			...
			var_01.bot_team = var_01.inliveplayerkillstreak;
...
watch_players_connecting()
{
	for(;;)
	{
		...
		if(!isai(var_00) && level.var_328.size > 0)
		{
...
bots_notify_on_spawn(param_00)
{
	...
	while(!common_scripts\utility::array_contains(level.var_328,param_00))
	{
		wait(0.05);
	}
...
bot_monitor_team_limits()
{
	...
	for(;;)
	{
		...
		foreach(var_02 in level.var_328)
		{
			...
			{
				level.bot_max_players_on_team[var_02.inliveplayerkillstreak]++;
			}
		}
...
update_max_players_from_team_agents()
{
	if(isdefined(level.agentarray))
	{
		foreach(var_01 in level.agentarray)
		{
			...
			{
				if(maps\mp\_utility::isteamparticipant(var_01) && isdefined(var_01.inliveplayerkillstreak) && var_01.inliveplayerkillstreak == "allies" || var_01.inliveplayerkillstreak == "axis")
				{
					level.bot_max_players_on_team[var_01.inliveplayerkillstreak]++;
				}
			}
		}
	}
...
bot_get_player_team()
{
	...
	{
		return self.inliveplayerkillstreak;
	}

	if(isdefined(self.connectedpostgame["team"]))
	{
		return self.connectedpostgame["team"];
	}
...
{
	if(isdefined(param_00.inliveplayerkillstreak) && param_00.inliveplayerkillstreak != "spectator")
	{
		return 1;
	}
...
bot_client_counts()
{
	...
	for(var_01 = 0;var_01 < level.var_328.size;var_01++)
	{
		...
		if(isdefined(var_02) && isdefined(var_02.inliveplayerkillstreak))
		{
			var_00 = cat_array_add(var_00,"all");
			var_00 = cat_array_add(var_00,var_02.inliveplayerkillstreak);
			if(isbot(var_02))
			{
				var_00 = cat_array_add(var_00,"bots");
				var_00 = cat_array_add(var_00,"bots_" + var_02.inliveplayerkillstreak);
				continue;
			}

			var_00 = cat_array_add(var_00,"humans");
			var_00 = cat_array_add(var_00,"humans_" + var_02.inliveplayerkillstreak);
		}
	}
...
move_bots_from_team_to_team(param_00,param_01,param_02,param_03)
{
	...
	{
		if(!isdefined(var_05.inliveplayerkillstreak))
		{
			continue;
		}

		if(isdefined(var_05.connected) && var_05.connected && isbot(var_05) && var_05.inliveplayerkillstreak == param_01)
		{
...
bots_update_difficulty(param_00,param_01)
{
	...
	{
		if(!isdefined(var_03.inliveplayerkillstreak))
		{
			continue;
		}

		if(isdefined(var_03.connected) && var_03.connected && isbot(var_03) && var_03.inliveplayerkillstreak == param_00)
		{
...
bot_drop()
{
	kickplayer(self.entity_number,"EXE_PLAYERKICKED_BOT_BALANCE");
	wait(0.1);
}
...
drop_bots(param_00,param_01)
{
	...
	foreach(var_04 in level.var_328)
	{
		if(isdefined(var_04.connected) && var_04.connected && isbot(var_04) && !isdefined(param_01) || isdefined(var_04.inliveplayerkillstreak) && var_04.inliveplayerkillstreak == param_01)
		{
...
spawn_bots(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	var_07 = [];
	var_08 = var_07.size;
	while(level.var_328.size < maps\mp\bots\_bots_util::bot_get_client_limit() && var_07.size < param_00 && gettime() < var_06)
	{
...
bot_think()
{
	...
	{
		self.bot_team = self.connectedpostgame["team"];
	}
	...
	if(!isdefined(self.bot_spawned_before))
	{
		...
		if(!bot_gametype_chooses_team())
		{
			var_02 = self.connectedpostgame["team"] != "spectator" && !isdefined(self.bot_team);
			if(!var_02)
			{
...
respawn_watcher()
{
	...
	if(maps\mp\gametypes\_playerlogic::needsbuttontorespawn())
	{
		while(self.waitingtospawn)
		{
			...
			{
				if(getdvarint("numlives") == 0 || self.connectedpostgame["lives"] > 0)
				{
...
bot_get_rank_xp_and_prestige()
{
	...
	if(!bot_israndom())
	{
		...
		{
			self.connectedpostgame["rankxp"] = 0;
		}

		if(!isdefined(self.connectedpostgame["prestige"]))
		{
			self.connectedpostgame["prestige"] = 0;
		}

		var_00.rankxp = self.connectedpostgame["rankxp"];
		var_00.prestige = self.connectedpostgame["prestige"];
		return var_00;
	}
	...
	var_04 = self.connectedpostgame[var_02];
	var_05 = self.connectedpostgame[var_03];
	var_06 = undefined;
	if(isdefined(var_04))
	{
	...
	else
	{
		...
		self.connectedpostgame[var_02] = var_0A;
	...
	else
	{
		...
		self.connectedpostgame[var_03] = var_0B;
...
bot_3d_sighting_model_thread(param_00)
{
	...
	for(;;)
	{
		if(isalive(self) && !self botcanseeentity(param_00) && common_scripts\utility::within_fov(self.var_2E6,self.var_41,param_00.var_2E6,self botgetfovdot()))
		{
			self botgetimperfectenemyinfo(param_00,param_00.var_2E6);
		}
...
get_human_player()
{
	...
	if(isdefined(var_01))
	{
		...
		{
			if(isdefined(var_01[var_02]) && isdefined(var_01[var_02].connected) && var_01[var_02].connected && !isai(var_01[var_02]) && !isdefined(var_00) || var_00.inliveplayerkillstreak == "spectator")
			{
...
bot_damage_callback(param_00,param_01,param_02,param_03,param_04,param_05)
{
	...
	if(isdefined(param_04))
	{
		if(level.teambased)
		{
			if(isdefined(param_04.inliveplayerkillstreak) && param_04.inliveplayerkillstreak == self.inliveplayerkillstreak)
			{
				return;
			}
			else if(isdefined(param_00) && isdefined(param_00.inliveplayerkillstreak) && param_00.inliveplayerkillstreak == self.inliveplayerkillstreak)
			{
				return;
			}
		}
...
bot_restart_think_threads()
{
	...
	thread maps\mp\bots\_bots_ks::func_1713();
	thread maps\mp\bots\_bots_ks::func_1718();
...
bot_think_seek_dropped_weapons()
{
	...
	for(;;)
	{
		...
		if(maps\mp\bots\_bots_util::bot_out_of_ammo() && self [[ level.bot_funcs["should_pickup_weapons"] ]]() && !maps\mp\bots\_bots_util::bot_is_remote_or_linked())
		{
			...
			var_02 = common_scripts\utility::get_array_of_closest(self.var_2E6,var_01);
			if(var_02.size > 0)
			{
				var_03 = var_02[0];
...
bot_seek_dropped_weapon(param_00)
{
	if(maps\mp\bots\_bots_strategy::bot_has_tactical_goal("seek_dropped_weapon",param_00) == 0)
	{
		...
		var_07.end_thread = 12;
		var_07.should_abort = level.bot_funcs["dropped_weapon_cancel"];
		var_07.action_thread = var_01;
		maps\mp\bots\_bots_strategy::bot_new_tactical_goal("seek_dropped_weapon",param_00.var_2E6,100,var_07);
	}
}
...
bot_think_level_actions(param_00)
{
	...
	for(;;)
	{
		...
		if(!isdefined(var_01))
		{
			...
			var_05 = common_scripts\utility::get_array_of_closest(self.var_2E6,level.level_specific_bot_targets);
			var_06 = 256;
			if(isdefined(param_00))
			{
				var_06 = param_00;
			...
			if(distancesquared(self.var_2E6,var_05[0].var_2E6) > var_06 * var_06)
			{
				continue;
			}
		...
		if(var_01.bot_interaction_type == "damage")
		{
			...
			if(var_07)
			{
				var_08 = var_01.var_2E6[2] - var_01.bot_targets[0].var_2E6[2] + 55;
				var_09 = var_01.var_2E6[2] - var_01.bot_targets[1].var_2E6[2] + 55;
			...
			if(var_0A == "spread")
			{
				var_0B = var_01.bot_targets[0].var_2E6 - var_01.var_2E6;
				var_0C = var_01.bot_targets[1].var_2E6 - var_01.var_2E6;
		...
		if(var_01.bot_interaction_type == "use")
		{
			...
			var_0F.script_goal_yaw = vectortoangles(var_01.var_2E6 - var_01.bot_target.var_2E6)[1];
			maps\mp\bots\_bots_strategy::bot_new_tactical_goal("map_interactive_object",var_01.bot_target.var_2E6,10,var_0F);
			continue;
		}
		...
		if(var_01.bot_interaction_type == "damage")
		{
			if(var_07)
			{
				...
				var_0F.end_thread = 20;
			}
			else
			{
				var_0F.action_thread = ::attack_damage_trigger;
				var_0F.end_thread = 50;
			}

			var_10 = undefined;
			var_11 = maps\mp\bots\_bots_util::bot_queued_process("GetPathDistLevelAction",::maps\mp\bots\_bots_util::func_get_path_dist,self.var_2E6,var_01.bot_targets[0].var_2E6);
			var_12 = maps\mp\bots\_bots_util::bot_queued_process("GetPathDistLevelAction",::maps\mp\bots\_bots_util::func_get_path_dist,self.var_2E6,var_01.bot_targets[1].var_2E6);
			if(!isdefined(var_01))
			{
				continue;
			}
			...
			maps\mp\bots\_bots_strategy::bot_new_tactical_goal("map_interactive_object",var_10.var_2E6,10,var_0F);
		}
	}
}
...
monitor_node_visible(param_00)
{
	...
	for(;;)
	{
		...
		{
			if(distancesquared(self.var_2E6,param_00.var_2E6) > 90000)
			{
				wait(0.05);
				continue;
			}
		...
		var_01 = self method_8387();
		if(isdefined(var_01))
		{
			if(function_01FF(var_01,param_00,1))
			{
				if(sighttracepassed(self.var_2E6 + (0,0,55),param_00.var_2E6 + (0,0,55),0,self))
				{
					self notify("goal");
				}
			}
...
{
	if(param_00.object.var_2E6[2] - self geteye()[2] > 55)
	{
		if(distance2dsquared(param_00.object.var_2E6,self.var_2E6) < 225)
		{
			return;
		}
	}
...
look_at_damage_trigger(param_00,param_01)
{
	...
	if(distance2dsquared(self.var_2E6,var_02) < 100)
	{
...
crate_in_range(param_00)
{
	...
	{
		if(distancesquared(self.var_2E6,param_00.var_2E6) > 4194304)
		{
			return 0;
		}
	}
...
bot_crate_valid(param_00)
{
	...
	if(level.teambased && isdefined(param_00.bomb) && isdefined(param_00.inliveplayerkillstreak) && param_00.inliveplayerkillstreak == self.inliveplayerkillstreak)
	{
		return 0;
	}
...
crate_has_landed(param_00)
{
	if(isdefined(param_00.boxtype))
	{
		return gettime() > param_00.var_7D + 1000;
	}
...
node_within_use_radius_of_crate(param_00,param_01)
{
	...
	{
		return abs(param_00.var_2E6[0] - param_01.var_2E6[0]) < 36 && abs(param_00.var_2E6[0] - param_01.var_2E6[0]) < 36 && abs(param_00.var_2E6[0] - param_01.var_2E6[0]) < 18;
	}

	var_02 = getdvarfloat("player_useRadius");
	var_03 = distancesquared(param_01.var_2E6,param_00.var_2E6 + (0,0,40));
...
crate_calculate_on_path_grid(param_00)
{
	...
	else
	{
		...
		var_05 = getnodesinradiussorted(param_00.var_2E6,var_04 * 2,0)[0];
		var_06 = param_00 getpointinbounds(0,0,-1);
		var_07 = undefined;
		if(isdefined(param_00.boxtype) && param_00.boxtype == "scavenger_bag")
		{
			if(maps\mp\bots\_bots_util::bot_point_is_on_pathgrid(param_00.var_2E6))
			{
		...
		{
			var_07 = botgetclosestnavigablepoint(param_00.var_2E6,var_04);
		}

		if(isdefined(var_05) && !var_05 nodeisdisconnected() && isdefined(var_07) && abs(var_05.var_2E6[2] - var_06[2]) < 30)
		{
...
{
	var_01 = getnodesinradiussorted(param_00.var_2E6,256,0);
	for(var_02 = var_01.size;var_02 > 0;var_02--)
	{
		var_01[var_02] = var_01[var_02 - 1];
	}

	var_01[0] = function_00C5(param_00.var_2E6);
	var_03 = undefined;
	if(isdefined(param_00.forcedisconnectuntil))
	{
		var_03 = getallnodes();
	...
	for(var_02 = 0;var_02 < var_01.size;var_02++)
	{
		...
		if(sighttracepassed(param_00.var_2E6,var_06.var_2E6 + (0,0,55),0,param_00))
		{
			wait(0.05);
			if(!isdefined(param_00))
			{
			...
			while(!isdefined(var_07) && var_08 < 100)
			{
				...
				if(distancesquared(var_06.var_2E6,var_09.var_2E6) > 250000)
				{
					var_07 = var_09;
				}
			}
			...
			if(isdefined(var_07))
			{
				var_0A = maps\mp\bots\_bots_util::bot_queued_process("GetNodesOnPathCrate",::maps\mp\bots\_bots_util::func_get_nodes_on_path,var_06.var_2E6,var_07.var_2E6);
				if(isdefined(var_0A))
				{
					var_04[var_04.size] = var_06;
					if(var_04.size == var_05)
...
crate_get_bot_target(param_00)
{
	...
	if(isdefined(param_00.nearest_nodes) && param_00.nearest_nodes.size > 0)
	{
		...
		return common_scripts\utility::random_weight_sorted(var_01).var_2E6;
	}
}
...
bot_think_crate()
{
	...
	for(;;)
	{
		...
		var_06 = common_scripts\utility::get_array_of_closest(self.var_2E6,var_06);
		var_0A = self method_8387();
		if(!isdefined(var_0A))
		{
			continue;
		}
		...
		foreach(var_08 in var_06)
		{
			...
			if((!isdefined(var_08.owner) || var_08.owner != self) && !isdefined(var_08.boxtype))
			{
				...
				{
					if(!isdefined(var_12.inliveplayerkillstreak))
					{
						continue;
					}

					if(!isai(var_12) && level.teambased && var_12.inliveplayerkillstreak == self.inliveplayerkillstreak)
					{
						if(distancesquared(var_12.var_2E6,var_08.var_2E6) < 490000)
						{
				...
				if(var_10.size > 0)
				{
					var_14 = var_10[0] method_8387();
					if(isdefined(var_14))
					{
						var_0F = 0;
						foreach(var_16 in var_08.nearest_nodes)
						{
							var_0F = var_0F | function_01FF(var_14,var_16,1);
						}
					}
				}
			}
			...
			if(!var_0F)
			{
				var_18 = isdefined(var_08.bots) && isdefined(var_08.bots[self.inliveplayerkillstreak]) && var_08.bots[self.inliveplayerkillstreak] > 0;
				var_19 = 0;
				foreach(var_16 in var_08.nearest_nodes)
				{
					var_19 = var_19 | function_01FF(var_0A,var_16,1);
				}
		...
		if(isdefined(var_0D))
		{
			if(self [[ level.bot_funcs["crate_should_claim"] ]]())
			{
				if(!isdefined(var_0D.boxtype))
				{
					...
					var_0D.bots[self.inliveplayerkillstreak] = 1;
				}
			}

			var_1D = spawnstruct();
			var_1D.object = var_0D;
			var_1D.var_8CBD = ::watch_bot_died_during_crate;
			...
			if(isdefined(var_0D.boxtype))
			{
				...
				{
					var_1D.end_thread = 16;
					var_1D.action_thread = undefined;
					var_1E = var_0D.var_2E6;
				}
				else
				{
					var_1D.end_thread = 50;
					var_1D.action_thread = ::use_box;
					var_1F = crate_get_bot_target(var_0D) - var_0D.var_2E6;
					var_20 = length(var_1F) * randomfloat(1);
					var_1E = var_0D.var_2E6 + vectornormalize(var_1F) * var_20 + (0,0,12);
				}
			}
			else
			{
				var_1D.action_thread = ::use_crate;
				var_1D.var_3150 = ::stop_using_crate;
				var_1E = crate_get_bot_target(var_0D);
				if(!maps\mp\killstreaks\_juggernaut::is_goliath_drop_pod(var_0D))
				{
					var_1D.end_thread = var_00 - distance(var_0D.var_2E6,var_1E + (0,0,40));
				}

				var_1E = var_1E + (0,0,24);
			}

			if(isdefined(var_1D.end_thread))
			{
			}
...
bot_should_use_scavenger_bag(param_00)
{
	if(maps\mp\bots\_bots_util::bot_get_low_on_ammo(0.66))
	{
		...
		{
			if(function_01FF(var_01,param_00.nearest_nodes[0],1))
			{
				if(common_scripts\utility::within_fov(self.var_2E6,self.var_41,param_00.var_2E6,self botgetfovdot()))
				{
					return 1;
				}
			}
...
crate_monitor_position()
{
	...
	for(;;)
	{
		...
		if(!maps\mp\bots\_bots_util::bot_vectors_are_equal(self.var_2E6,var_00))
		{
...
use_crate(param_00)
{
	if(maps\mp\killstreaks\_juggernaut::is_goliath_drop_pod(param_00.object))
	{
		...
		self botlookatpoint(param_00.object.var_2E6 + (0,0,self getviewheight()),level.cratenonownerusetime / 1000 + 1,"script_forced");
	}
...
use_box(param_00)
{
	...
	if(isdefined(param_00.object) && isdefined(param_00.object.boxtype))
	{
		...
		if(isdefined(param_00.object))
		{
			var_02 = level.boxsettings[param_00.object.boxtype].var_9C11 / 1000 + 0.5;
...
stop_using_crate(param_00)
{
	...
	if(isdefined(param_00.object))
	{
		param_00.object.bots[self.inliveplayerkillstreak] = 0;
	}
}
...
bot_think_crate_blocking_path()
{
	...
	for(;;)
	{
		...
		for(var_02 = 0;var_02 < var_01.size;var_02++)
		{
			...
			{
				if(!maps\mp\_utility::isjuggernaut() && distancesquared(self.var_2E6,var_03.var_2E6) < 3600)
				{
					var_04 = spawnstruct();
			...
			var_05 = self method_84C5();
			if(!isdefined(var_05) || var_05 != var_03)
			{
				continue;
			}

			if(distancesquared(self.var_2E6,var_03.var_2E6) < var_00 * var_00)
			{
...
bot_think_revive()
{
	...
	for(;;)
	{
		...
		if(var_01.size > 1)
		{
			var_01 = sortbydistance(var_01,self.var_2E6);
		...
		for(var_02 = 0;var_02 < var_01.size;var_02++)
		{
			...
			if(isdefined(var_05.bots) && isdefined(var_05.bots[self.inliveplayerkillstreak]) && var_05.bots[self.inliveplayerkillstreak] > 0)
			{
				continue;
			}
			...
			if(distancesquared(self.var_2E6,var_05.var_2E6) < 4194304)
			{
				var_06 = spawnstruct();
				var_06.object = var_04;
				var_06.end_thread = 64;
				if(isdefined(self.start_thread) && gettime() - self.start_thread < 1000)
				{
					var_06.end_thread = 32;
				}

				var_06.var_8CBD = ::watch_bot_died_during_revive;
				var_06.var_3150 = ::stop_reviving;
				var_06.should_abort = ::player_revived_or_dead;
				var_06.action_thread = ::revive_player;
				maps\mp\bots\_bots_strategy::bot_new_tactical_goal("revive",var_05.var_2E6,60,var_06);
				break;
			}
		}
	}
...
stop_reviving(param_00)
{
	...
	{
		param_00.object.owner.bots[self.inliveplayerkillstreak] = 0;
	}
}
...
{
	if(distancesquared(self.var_2E6,param_00.object.owner.var_2E6) > 4096)
	{
		self.start_thread = gettime();
		return;
	}
...
bot_know_enemies_on_start()
{
	...
	for(var_02 = 0;var_02 < level.var_328.size;var_02++)
	{
		...
		if(isdefined(var_03) && isdefined(self.inliveplayerkillstreak) && isdefined(var_03.inliveplayerkillstreak) && !function_0285(self,var_03))
		{
...
bot_make_entity_sentient(param_00,param_01)
{
	if(isdefined(param_01))
	{
		return self sentient_makeentitysentient(param_00,param_01);
	}

	return self sentient_makeentitysentient(param_00);
}
...
bot_triggers()
{
	...
	foreach(var_02 in var_00)
	{
		...
		var_02 thread bot_flag_trigger(var_02.var_39B);
	}
}
