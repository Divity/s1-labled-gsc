 ...
 * Platform: PC
 * Function Count: 50
 * Decompile Time: 2931 ms
 * Timestamp: 4/20/2022 8:13:21 PM
*******************************************************************/
...
bot_defend_get_random_entrance_point_for_current_area()
{
	...
	{
		return common_scripts\utility::random(var_00).var_2E6;
	}
...
bot_get_ambush_trap_item(param_00,param_01,param_02)
{
	...
	var_07 = isdefined(var_05) && self getweaponammoclip(var_05) > 0 || self getweaponammostock(var_05) > 0;
	var_08 = isdefined(var_06) && self getweaponammoclip(var_06) > 0 || self getweaponammostock(var_06) > 0;
	if(!var_07 && !var_08)
	{
		return;
	}
...
bot_set_ambush_trap(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(!isdefined(param_04) && isdefined(param_01) && param_01.size > 0)
	{
		...
		foreach(var_09 in param_01)
		{
			...
			if(distancesquared(var_09.var_2E6,param_02.var_2E6) > 90000)
			{
				if(vectordot(var_07,vectornormalize(var_09.var_2E6 - param_02.var_2E6)) < 0.4)
				{
					var_06[var_06.size] = var_09;
				}
			}
		...
		if(var_06.size > 0)
		{
			...
			var_0B = getnodesinradius(var_05.var_2E6,300,50);
			var_0C = [];
	...
	if(isdefined(param_04))
	{
		...
		{
			var_11 = vectortoangles(var_05.var_2E6 - param_04.var_2E6);
			var_10 = var_11[1];
		}
		...
		if(var_12)
		{
			...
			if(var_13 == "goal")
			{
				...
				if(!isdefined(self.enemy) || 0 == self botcanseeentity(self.enemy))
				{
					if(isdefined(var_10))
					{
						self botthrowgrenade(var_05.var_2E6,param_00["item_action"]);
					}
					else
					{
						self botthrowgrenade(self.var_2E6 + anglestoforward(self.var_41) * 50,param_00["item_action"]);
					}
...
bot_watch_manual_detonate(param_00,param_01,param_02)
{
	...
	for(;;)
	{
		if(distancesquared(self.var_2E6,param_00.var_2E6) > var_03)
		{
			var_04 = self sentient_getclosestenemysqdist(param_00.var_2E6,1);
			if(var_04 < var_03)
			{
...
bot_defend_think(param_00,param_01,param_02,param_03)
{
	...
	if(isdefined(param_03))
	{
		...
		if(isdefined(param_03["override_entrances"]) && param_03["override_entrances"].size > 0)
		{
			...
			{
				var_08.prone_visible_from[self.defend_entrance_index] = maps\mp\bots\_bots_util::entrance_visible_from(var_08.var_2E6,maps\mp\bots\_bots_util::defend_valid_center(),"prone");
				wait(0.05);
				var_08.crouch_visible_from[self.defend_entrance_index] = maps\mp\bots\_bots_util::entrance_visible_from(var_08.var_2E6,maps\mp\bots\_bots_util::defend_valid_center(),"crouch");
				wait(0.05);
			}
		}
	...
	if(!isdefined(self.bot_defend_player_guarding))
	{
		...
		if(!isdefined(var_0A))
		{
			var_0A = function_00C5(maps\mp\bots\_bots_util::defend_valid_center());
		}
		...
		if(!isdefined(var_0A))
		{
			...
			{
				var_0E = vectornormalize(var_0C[var_0D].var_2E6 - var_0B);
				var_0F = var_0B + var_0E * 15;
				if(sighttracepassed(var_0F,var_0C[var_0D].var_2E6,0,undefined))
				{
				...
				wait(0.05);
				if(sighttracepassed(var_0F + (0,0,55),var_0C[var_0D].var_2E6 + (0,0,55),0,undefined))
				{
					var_0A = var_0C[var_0D];
					break;
				}
	...
	for(;;)
	{
		...
		{
			if(isdefined(level.aerial_danger_exists_for) && level.aerial_danger_exists_for[self.inliveplayerkillstreak])
			{
...
should_start_cautious_approach_default(param_00)
{
	...
	if(param_00)
	{
		...
		return distancesquared(self.var_2E6,self.bot_defending_center) > var_02 * 0.75 * 0.75;
	}

	if(self botpursuingscriptgoal() && distancesquared(self.var_2E6,self.bot_defending_center) < var_02)
	{
...
defense_cautious_approach()
{
	...
	for(var_17 = 1;var_17 < var_05.size - 2;var_17++)
	{
		...
		var_18 = function_0210(var_05[var_17]);
		if(var_18.size == 0)
		{
			continue;
		}
		...
		{
			if(!common_scripts\utility::within_fov(self.var_2E6,self.var_41,var_18[var_0C].var_2E6,0))
			{
				continue;
			}
			...
			for(var_1A = 0;var_1A < self.locations_to_investigate.size;var_1A++)
			{
				...
				if(function_01FF(var_10.node,var_18[var_0C],1))
				{
		...
		if(isdefined(var_1B))
		{
			...
			{
				if(function_01FF(self.locations_to_investigate[var_0C].node,var_1B,1))
				{
					if(distance2dsquared(self.locations_to_investigate[var_0C].var_2E6,var_1B.var_2E6) > 3600)
					{
			...
			if(var_1D == "goal")
			{
				...
				{
					if(distance2dsquared(self.var_2E6,var_1C[var_0C].var_2E6) < 1600)
					{
						continue;
					}
					...
					{
						self botlookatpoint(var_1C[var_0C].var_2E6 + (0,0,self getviewheight()),0.25,"script_search");
						wait(0.25);
						var_1E = var_1E + 0.25;
					}
				}
...
monitor_cautious_approach_early_out()
{
	...
	for(;;)
	{
		if(distancesquared(self.var_2E6,self.bot_defending_center) < var_00)
		{
...
monitor_cautious_approach_dangerous_locations()
{
	...
	for(;;)
	{
		...
		if(isdefined(var_01))
		{
			...
			{
				if(function_01FF(var_01,self.locations_to_investigate[var_03].node,1))
				{
					var_04 = common_scripts\utility::within_fov(self.var_2E6,self.var_41,self.locations_to_investigate[var_03].var_2E6,var_02);
					var_05 = !var_04 || self.locations_to_investigate[var_03].frames_visible < 17;
					if(var_05 && distancesquared(self.var_2E6,self.locations_to_investigate[var_03].var_2E6) < var_00)
					{
						var_04 = 1;
	...
	for(;;)
	{
		...
		foreach(var_07 in var_05)
		{
			...
			if(var_07.health == 0 && isdefined(var_07.deathtime) && var_04 - var_07.deathtime < 5000)
			{
				...
				{
					if(isdefined(var_07.lastattacker) && isdefined(var_07.lastattacker.inliveplayerkillstreak) && var_07.lastattacker.inliveplayerkillstreak == common_scripts\utility::get_enemy_team(self.inliveplayerkillstreak))
					{
						if(distancesquared(var_07.var_2E6,self.var_2E6) < var_02)
						{
							self botgetimperfectenemyinfo(var_07.lastattacker,var_07.var_2E6);
							var_09 = function_00C5(var_07.var_2E6);
							if(isdefined(var_09))
							{
								self.defense_investigate_specific_point = var_09.var_2E6;
...
defense_get_initial_entrances()
{
	...
	{
		var_00 = findentrances(self.var_2E6);
		return var_00;
	}
}
...
defense_watch_entrances_at_goal()
{
	...
	if(maps\mp\bots\_bots_util::bot_is_capturing())
	{
		...
		if(isdefined(var_00))
		{
			...
			{
				if(function_01FF(var_00,var_04,1))
				{
		...
		if(var_01.size == 0)
		{
			var_01 = findentrances(self.var_2E6);
		}
	}
	else if(maps\mp\bots\_bots_util::bot_is_protecting() || maps\mp\bots\_bots_util::bot_is_bodyguarding())
	{
		...
		{
			if(function_01FF(var_00,self.node_closest_to_defend_center,1))
			{
...
bot_monitor_watch_entrances_bodyguard()
{
	...
	for(;;)
	{
		var_00 = anglestoforward(self.bot_defend_player_guarding.var_41) * (1,1,0);
		var_00 = vectornormalize(var_00);
		foreach(var_02 in self.watch_nodes)
		{
			var_02.watch_node_chance[self.entity_number] = 1;
			var_03 = var_02.var_2E6 - self.bot_defend_player_guarding.var_2E6;
...
entrance_to_enemy_zone(param_00)
{
	...
	{
		if(botzonegetcount(var_03,self.inliveplayerkillstreak,"enemy_predict") > 0)
		{
			...
			{
				var_04 = vectornormalize(getzoneorigin(var_03) - self.var_2E6);
...
prioritize_watch_nodes_toward_enemies(param_00)
{
	...
	{
		if(botzonegetcount(var_02,self.inliveplayerkillstreak,"enemy_predict") <= 0)
		{
			continue;
		}
		...
		var_03 = vectornormalize(getzoneorigin(var_02) - self.var_2E6);
		for(var_04 = 0;var_04 < var_01.size;var_04++)
		{
			...
			{
				var_07 = vectornormalize(var_01[var_04].var_2E6 - self.var_2E6);
...
entrance_watched_by_player(param_00,param_01)
{
	...
	var_03 = vectornormalize(param_01.var_2E6 - param_00.var_2E6);
...
bot_get_teammates_in_radius(param_00,param_01)
{
	...
	for(var_04 = 0;var_04 < level.participants.size;var_04++)
	{
		...
		if(var_05 != self && isdefined(var_05.inliveplayerkillstreak) && var_05.inliveplayerkillstreak == self.inliveplayerkillstreak && maps\mp\_utility::isteamparticipant(var_05))
		{
			if(distancesquared(param_00,var_05.var_2E6) < var_02)
			{
...
defend_get_ally_bots_at_zone_for_stance(param_00)
{
	...
	foreach(var_03 in level.participants)
	{
		...
		if(var_03.inliveplayerkillstreak == self.inliveplayerkillstreak && var_03 != self && isai(var_03) && var_03 maps\mp\bots\_bots_util::bot_is_defending() && var_03.cur_defend_stance == param_00)
		{
...
monitor_defend_player()
{
	var_00 = 0;
	var_01 = 175;
	var_02 = self.bot_defend_player_guarding.var_2E6;
	var_03 = 0;
	var_04 = 0;
	for(;;)
	{
		...
		self.bot_defending_center = self.bot_defend_player_guarding.var_2E6;
		self.node_closest_to_defend_center = self.bot_defend_player_guarding method_8387();
		if(!isdefined(self.node_closest_to_defend_center))
		{
			self.node_closest_to_defend_center = self method_8387();
		}
		...
		if(self botgetscriptgoaltype() != "none")
		{
			...
			{
				var_00 = 0;
				if(distancesquared(var_02,self.bot_defend_player_guarding.var_2E6) > var_01 * var_01)
				{
					var_02 = self.bot_defend_player_guarding.var_2E6;
					var_04 = 1;
					var_08 = vectornormalize(var_05 - self.bot_defend_player_guarding.var_2E6);
			...
			var_03 = var_07;
			if(abs(self.bot_defend_player_guarding.var_2E6[2] - var_05[2]) >= 50)
			{
				self notify("defend_force_node_recalculation");
				wait(0.25);
			}
...
find_defend_node_capture()
{
	...
	if(isdefined(var_01))
	{
		if(isdefined(var_00))
		{
			var_02 = vectornormalize(var_00 - var_01.var_2E6);
			self.cur_defend_angle_override = vectortoangles(var_02);
		}
		else
		{
			var_03 = vectornormalize(var_01.var_2E6 - maps\mp\bots\_bots_util::defend_valid_center());
...
find_defend_node_protect()
{
	...
	if(isdefined(var_00))
	{
		var_01 = vectornormalize(maps\mp\bots\_bots_util::defend_valid_center() - var_00.var_2E6);
...
find_defend_node_bodyguard()
{
	...
	self.cur_defend_point_override = self.var_2E6;
}
...
bot_check_team_is_using_position(param_00,param_01,param_02,param_03)
{
	...
	{
		var_05 = level.participants[var_04];
		if(var_05.inliveplayerkillstreak == self.inliveplayerkillstreak && var_05 != self)
		{
			if(isai(var_05))
			{
				if(param_02)
				{
					if(distancesquared(param_00,var_05.var_2E6) < 441)
					{
						return 1;
					}
				}
...
bot_capture_zone_get_furthest_distance()
{
	...
	if(isdefined(self.bot_defending_nodes))
	{
		...
		{
			var_03 = distance(self.bot_defending_center,var_02.var_2E6);
	...
	for(;;)
	{
		if(self.tactical_goals.size > 0 && !maps\mp\bots\_bots_util::bot_is_remote_or_linked())
		{
			...
			if(!isdefined(var_00.abort))
			{
				...
				if(isdefined(var_00.var_8CBD))
				{
					self [[ var_00.var_8CBD ]](var_00);
				}
				...
				if(isdefined(var_00.var_3150))
				{
					self [[ var_00.var_3150 ]](var_00);
				}
			}
...
bot_new_tactical_goal(param_00,param_01,param_02,param_03)
{
	...
	if(isdefined(param_03.end_thread))
	{
		var_04.goal_radius = param_03.end_thread;
	}

	var_04.var_8CBD = param_03.var_8CBD;
	var_04.var_3150 = param_03.var_3150;
...
bot_melee_tactical_insertion_check()
{
	...
	if(!isdefined(self.last_melee_ti_check) || var_00 - self.last_melee_ti_check > 1000)
	{
		...
		var_02 = getzonenearest(self.var_2E6);
		if(!isdefined(var_02))
		{
			return 0;
		}

		var_03 = botzonenearestcount(var_02,self.inliveplayerkillstreak,1,"enemy_predict",">",0);
		if(!isdefined(var_03))
		{
			return 0;
		}

		var_04 = getnodesinradius(self.var_2E6,500,0);
		if(var_04.size <= 0)
		{
			return 0;
		}
