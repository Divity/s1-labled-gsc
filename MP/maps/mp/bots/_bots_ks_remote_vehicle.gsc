 ...
 * Platform: PC
 * Function Count: 23
 * Decompile Time: 1256 ms
 * Timestamp: 4/20/2022 8:13:13 PM
*******************************************************************/
...
bot_killstreak_remote_control(param_00,param_01,param_02,param_03,param_04)
{
	...
	if(isdefined(self.node_ambushing_from))
	{
		...
		var_09 = distancesquared(self.var_2E6,self.node_ambushing_from.var_2E6);
	...
	if(var_0A || var_05)
	{
		var_0B = getnodesinradius(self.var_2E6,500,0,512);
		if(isdefined(var_0B) && var_0B.size > 0)
		{
			...
			if(var_0A)
			{
				...
				{
					if(bullettracepassed(var_0E.var_2E6 + (0,0,30),var_0E.var_2E6 + (0,0,400),0,self))
					{
						var_07 = var_0E;
						break;
					}
	...
	maps\mp\bots\_bots_ks::func_1708(param_00,param_01,param_00.weapon);
...
{
	return bot_killstreak_get_outside_players(self.inliveplayerkillstreak,"enemy",param_00);
}

//Function Number: 16
bot_killstreak_get_all_outside_allies(param_00)
{
	return bot_killstreak_get_outside_players(self.inliveplayerkillstreak,"ally",param_00);
}
...
bot_killstreak_get_outside_players(param_00,param_01,param_02)
{
	...
	foreach(var_06 in var_04)
	{
		...
		if(param_01 == "ally")
		{
			var_07 = level.teambased && param_00 == var_06.inliveplayerkillstreak;
		}
...
bot_control_heli_main_move_loop(param_00,param_01)
{
	...
	var_05 = find_closest_heli_node_2d(self.vehicle_controlling.var_2E6,param_00);
	var_06 = undefined;
	self.next_goal_time = 0;
	var_07 = "needs_new_goal";
	var_08 = undefined;
	var_09 = self.vehicle_controlling.var_2E6;
	var_0A = 3;
	var_0B = 0.05;
	while(self [[ level.bot_ks_funcs["isUsing"][param_00] ]]())
	{
		if(gettime() > self.next_goal_time && var_07 == "needs_new_goal")
		{
			...
			if(isdefined(var_05))
			{
				...
				if(param_01)
				{
					var_0E = var_05.var_2E6 + maps\mp\_utility::gethelipilotmeshoffset() + level.bot_heli_pilot_traceoffset;
					var_0F = var_05.var_2E6 + maps\mp\_utility::gethelipilotmeshoffset() - level.bot_heli_pilot_traceoffset;
		...
		if(var_07 == "waiting_till_goal")
		{
			if(!param_01)
			{
				var_11 = var_06[2] - self.vehicle_controlling.var_2E6[2];
				if(var_11 > 10)
				{
			...
			var_12 = var_06 - self.vehicle_controlling.var_2E6;
			if(param_01)
			{
			...
			else
			{
				...
				if(var_0A <= 0)
				{
					if(distancesquared(self.vehicle_controlling.var_2E6,var_09) < 225)
					{
...
get_random_outside_target()
{
	...
	{
		var_03 = botzonegetcount(var_02,self.inliveplayerkillstreak,"enemy_predict");
		if(var_03 > 0)
		{
	...
	else
	{
		...
		while(var_09 < 10)
		{
			...
			var_05 = var_0A.var_2E6;
			if(nodeexposedtosky(var_0A) && distance2dsquared(var_0A.var_2E6,self.vehicle_controlling.var_2E6) > 62500)
			{
				break;
			}
		}
