 ...
 * Platform: PC
 * Function Count: 74
 * Decompile Time: 4615 ms
 * Timestamp: 4/20/2022 8:13:26 PM
*******************************************************************/

//Function Number: 1
bot_get_nodes_in_cone(param_00,param_01,param_02,param_03)
{
	var_04 = getnodesinradius(self.var_2E6,param_01,param_00);
	var_05 = [];
	var_06 = self method_8387();
	var_07 = anglestoforward(self getplayerangles());
	var_08 = vectornormalize(var_07 * (1,1,0));
	foreach(var_0A in var_04)
	{
		var_0B = vectornormalize(var_0A.var_2E6 - self.var_2E6 * (1,1,0));
		var_0C = vectordot(var_0B,var_08);
		if(var_0C > param_02)
		{
			if(!param_03 || isdefined(var_06) && function_01FF(var_0A,var_06,1))
			{
...
bot_set_difficulty(param_00,param_01)
{
	...
	if(isplayer(self) && var_03 != param_00)
	{
		...
		var_04 = maps\mp\gametypes\_rank::getrankforxp(maps\mp\gametypes\_rank::gettotalxp());
		self.connectedpostgame["rank"] = var_04;
		var_05 = self.connectedpostgame["prestige"];
...
bot_choose_difficulty_for_default()
{
	...
	var_00 = self.connectedpostgame["bot_chosen_difficulty"];
	if(!isdefined(var_00))
	{
		var_01 = [];
		...
		foreach(var_04 in level.var_328)
		{
			...
			var_06 = var_04.inliveplayerkillstreak;
			if(!isdefined(var_06))
			{
			...
			if(!isdefined(var_06))
			{
				var_06 = var_04.connectedpostgame["team"];
			}
	...
	if(isdefined(var_00))
	{
		self.connectedpostgame["bot_chosen_difficulty"] = var_00;
	}
...
get_all_connected_nodes(param_00)
{
	...
	{
		var_03 = function_0210(param_00[var_02]);
...
get_visible_nodes_array(param_00,param_01)
{
	...
	{
		if(function_01FF(var_04,param_01,1))
		{
	...
	if(isarray(param_01))
	{
		if(isdefined(param_02) && param_02)
		{
			...
			foreach(var_06 in param_01)
			{
				...
				var_09 = distancesquared(self.var_2E6,var_08);
				if(var_09 < var_04)
				{
					var_03 = var_06;
					var_04 = var_09;
...
bot_find_node_to_guard_player(param_00,param_01,param_02)
{
	...
	if(lengthsquared(var_04) > 100)
	{
		...
		{
			var_09 = vectornormalize(var_05[var_08].var_2E6 - self.bot_defend_player_guarding.var_2E6);
	...
	if(isdefined(param_02) && param_02)
	{
		var_0A = vectornormalize(self.bot_defend_player_guarding.var_2E6 - self.var_2E6);
		var_0B = var_06;
		var_06 = [];
		foreach(var_0D in var_0B)
		{
			var_09 = vectornormalize(var_0D.var_2E6 - self.bot_defend_player_guarding.var_2E6);
	...
	{
		var_12 = distancesquared(var_06[var_08].var_2E6,param_00) > 10000;
		var_13 = abs(var_06[var_08].var_2E6[2] - self.bot_defend_player_guarding.var_2E6[2]) < 50;
		if(var_12)
		{
...
bot_pick_random_point_from_set(param_00,param_01,param_02)
{
	...
	if(!isdefined(var_03))
	{
		...
		var_05 = var_04.var_2E6 - param_00;
...
bot_find_random_midpoint(param_00,param_01)
{
	...
	for(var_04 = 0;var_04 < var_03.size;var_04++)
	{
		for(var_05 = var_04 + 1;var_05 < var_03.size;var_05++)
		{
			...
			if(function_01FF(var_06,var_07,1))
			{
				var_02 = (var_06.var_2E6[0] + var_07.var_2E6[0] * 0.5,var_06.var_2E6[1] + var_07.var_2E6[1] * 0.5,var_06.var_2E6[2] + var_07.var_2E6[2] * 0.5);
...
defend_valid_center()
{
	...
	{
		return self.bot_defending_override_origin_node.var_2E6;
	}
...
bot_allowed_to_use_killstreaks()
{
	...
	if(maps\mp\_utility::getgametypenumlives() > 0)
	{
		...
		foreach(var_02 in level.participants)
		{
			if(isalive(var_02) && !function_0285(self,var_02))
			{
				var_00 = 0;
			}
		}
...
bot_recent_point_of_interest()
{
	...
	if(isdefined(var_00))
	{
		...
		var_08 = getzonenearest(self.var_2E6);
		if(isdefined(var_07) && isdefined(var_08) && var_08 != var_07)
		{
			var_09 = botzonegetcount(var_07,self.inliveplayerkillstreak,"ally") + botzonegetcount(var_07,self.inliveplayerkillstreak,"path_ally");
			if(var_09 > 1)
			{
				var_00 = undefined;
			}
...
bot_watch_nodes(param_00,param_01,param_02,param_03,param_04,param_05,param_06,param_07)
{
	...
	while(var_08)
	{
		...
		{
			if(distancesquared(self botgetscriptgoal(),self.var_2E6) < var_09)
			{
	...
	if(isdefined(param_00))
	{
		...
		{
			var_0E = 0;
			if(distance2dsquared(self.var_2E6,var_0D.var_2E6) <= 40)
			{
				var_0E = 1;
			}

			var_0F = self geteye();
			var_10 = vectordot((0,0,1),vectornormalize(var_0D.var_2E6 + var_0B - var_0F));
			if(abs(var_10) > 0.92)
			{
				var_0E = 1;
			}
	...
	for(;;)
	{
		...
		if(self.watch_nodes.size > 0)
		{
			...
			if(isdefined(self.enemy))
			{
				var_1D = self sentient_lastknownpos(self.enemy);
				var_1E = self sentient_lastknowntime(self.enemy);
				if(var_1E && var_1A - var_1E < 5000)
				{
					var_1F = vectornormalize(var_1D - self.var_2E6);
					var_20 = 0;
					for(var_21 = 0;var_21 < self.watch_nodes.size;var_21++)
					{
						var_22 = vectornormalize(self.watch_nodes[var_21].var_2E6 - self.var_2E6);
			...
			if(!var_1C && var_1A >= var_15)
			{
				...
				for(var_21 = 0;var_21 < self.watch_nodes.size;var_21++)
				{
					...
					if(var_18 && !common_scripts\utility::within_fov(self.var_2E6,var_17,var_0D.var_2E6,param_02))
					{
						continue;
					}

					if(distance2dsquared(self.var_2E6,var_0D.var_2E6) <= 10)
					{
						continue;
					}
					...
					if(common_scripts\utility::within_fov(self.var_2E6,self.var_41,var_0D.var_2E6,var_1B))
					{
			...
			if(isdefined(var_19))
			{
				var_27 = var_19.var_2E6 + var_0B;
				if(distance2dsquared(self.var_2E6,var_27) <= 10)
				{
					self botlookatpoint(undefined);
					var_19 = undefined;
					var_15 = 0;
...
bot_leader_dialog(param_00,param_01)
{
	...
	{
		if(!common_scripts\utility::within_fov(self.var_2E6,self.var_41,param_01,self botgetfovdot()))
		{
...
bot_get_known_attacker(param_00,param_01)
{
	if(isdefined(param_01) && isdefined(param_01.classname))
	{
		...
		{
			if(isdefined(param_01.var_9CBC))
			{
				return param_01.var_9CBC;
			}
...
bot_get_zones_within_dist_recurs(param_00,param_01)
{
	...
	param_00.visited = 1;
	var_03 = function_0210(param_00);
	foreach(var_05 in var_03)
	{
		if(!var_05.visited)
		{
			var_06 = distance(param_00.var_2E6,var_05.var_2E6);
			if(var_06 < param_01)
			{
...
bot_monitor_enemy_camp_spots(param_00)
{
	...
	for(;;)
	{
		...
		foreach(var_03 in level.participants)
		{
			...
			if(var_03 [[ param_00 ]]() && !isdefined(var_01[var_03.inliveplayerkillstreak]))
			{
				level.enemy_camp_assassin[var_03.inliveplayerkillstreak] = undefined;
				level.enemy_camp_spots[var_03.inliveplayerkillstreak] = var_03 botpredictenemycampspots(1);
				if(isdefined(level.enemy_camp_spots[var_03.inliveplayerkillstreak]))
				{
					if(!isdefined(level.enemy_camp_assassin_goal[var_03.inliveplayerkillstreak]) || !common_scripts\utility::array_contains(level.enemy_camp_spots[var_03.inliveplayerkillstreak],level.enemy_camp_assassin_goal[var_03.inliveplayerkillstreak]))
					{
						level.enemy_camp_assassin_goal[var_03.inliveplayerkillstreak] = common_scripts\utility::random(level.enemy_camp_spots[var_03.inliveplayerkillstreak]);
					}

					if(isdefined(level.enemy_camp_assassin_goal[var_03.inliveplayerkillstreak]))
					{
						var_04 = [];
						foreach(var_06 in level.participants)
						{
							if(!isdefined(var_06.inliveplayerkillstreak))
							{
								continue;
							}

							if(var_06 [[ param_00 ]]() && var_06.inliveplayerkillstreak == var_03.inliveplayerkillstreak)
							{
								var_04[var_04.size] = var_06;
							}
						}

						var_04 = sortbydistance(var_04,level.enemy_camp_assassin_goal[var_03.inliveplayerkillstreak]);
						if(var_04.size > 0)
						{
							level.enemy_camp_assassin[var_03.inliveplayerkillstreak] = var_04[0];
						}
					}
				}

				var_01[var_03.inliveplayerkillstreak] = 1;
			}
		}
	}
}
...
bot_update_camp_assassin()
{
	...
	if(!isdefined(level.enemy_camp_assassin[self.inliveplayerkillstreak]))
	{
		return;
	}

	if(level.enemy_camp_assassin[self.inliveplayerkillstreak] == self)
	{
		maps\mp\bots\_bots_strategy::bot_defend_stop();
		self botsetscriptgoal(level.enemy_camp_assassin_goal[self.inliveplayerkillstreak],128,"objective",undefined,256);
