...
//Function Number: 1
lib_A7A7::timeuntilwavespawn(param_00)
{
	if(!self.afk)
	{
		return 0;
...
//Function Number: 3
lib_A7A7::timeuntilspawn(param_00)
{
	...
	if(self.afk)
	{
		...
		if(param_00)
		{
			...
			{
				var_01 = var_01 + lib_A7A7::teamkilldelay();
			}
	...
	if(var_04)
	{
		return lib_A7A7::timeuntilwavespawn(var_01);
	}
...
lib_A7A7::spawnclient()
{
	...
	if(!lib_A7A7::mayspawn())
	{
		...
		if(isdefined(var_00) && var_00)
		{
			...
			if(!self.afk && lib_A7A7::teamkilldelay() <= 0)
			{
		...
		else if(maps\mp\_utility::isroundbased() && !maps\mp\_utility::islastround())
		{
			...
			thread lib_A7A7::removespawnmessageshortly(3);
		}

		thread lib_A7A7::spawnspectator();
		return;
	}
...
//Function Number: 7
lib_A7A7::gatherclassweapons(param_00,param_01)
{
	var_02 = [];
	var_03 = param_01;
...
//Function Number: 8
lib_A7A7::streamclassweapons(param_00,param_01,param_02)
{
	...
	var_03 = lib_A7A7::gatherclassweapons(param_01,param_02);
	if(var_03.size > 0)
	{
...
lib_A7A7::waitandspawnclient()
{
	...
	{
		var_04 = lib_A7A7::teamkilldelay();
		if(var_04 > 0)
		{
			maps\mp\_utility::setlowermessage("friendly_fire",&"MP_FRIENDLY_FIRE_WILL_NOT",var_04,1,1);
			thread lib_A7A7::respawn_asspectator(var_02.var_2E6,var_02.var_41);
			var_00 = 1;
			wait(var_04);
	...
	else if(lib_A7A7::teamkilldelay())
	{
		self.connectedpostgame["teamkills"] = max(self.connectedpostgame["teamkills"] - 1,0);
	}
	...
	var_06 = lib_A7A7::timeuntilspawn(0);
	if(var_06 > 0)
	{
	...
	if(lib_A7A7::needsbuttontorespawn())
	{
		...
		var_00 = 1;
		lib_A7A7::waitrespawnbutton();
	}

	self.waitingtospawn = 0;
	maps\mp\_utility::clearlowermessage("spawn_info");
	self.var_A2A6 = undefined;
	thread lib_A7A7::spawnplayer();
}

//Function Number: 10
lib_A7A7::needsbuttontorespawn()
{
...
//Function Number: 12
lib_A7A7::removespawnmessageshortly(param_00)
{
	self endon("disconnect");
	level endon("game_ended");
	waittillframeend;
...
//Function Number: 13
lib_A7A7::laststandrespawnplayer()
{
...
//Function Number: 15
lib_A7A7::showspawnnotifies()
{
}

//Function Number: 16
lib_A7A7::getspawnorigin(param_00)
{
...
//Function Number: 18
lib_A7A7::spawningclientthisframereset()
{
...
//Function Number: 19
lib_A7A7::setuioptionsmenu(param_00)
{
	self endon("disconnect");
...
//Function Number: 21
lib_A7A7::spawnplayer(param_00,param_01)
{
	self endon("disconnect");
	self endon("joined_spectators");
	self notify("spawned");
	...
	var_02 = undefined;
	self.var_933D = 0;
	thread lib_A7A7::setuioptionsmenu(0);
	...
	if(!level.ingraceperiod && !self.hasdonecombat)
	{
		...
		thread lib_A7A7::spawningclientthisframereset();
	...
	else if(isdefined(self.var_800F) && isdefined(self.var_800F.var_623B) || lib_A7A7::tivalidationcheck())
	{
	...
	if(!param_00)
	{
		...
		thread lib_A7A7::listenforexomoveevent();
	...
	lib_A7A7::resetuidvarsonspawn();
	if(isdefined(var_02))
	{
		maps\mp\gametypes\_spawnlogic::finalizespawnpointchoice(var_02);
		var_05 = lib_A7A7::getspawnorigin(var_02);
		var_06 = var_02.var_41;
	}
	else
	{
	...
	if(!maps\mp\_utility::gameflag("prematch_done") || !var_0E && game["state"] == "playing")
	{
		...
		thread lib_A7A7::showspawnnotifies();
	}
...
//Function Number: 23
lib_A7A7::listenforexomoveevent()
{
	self endon("disconnect");
	self endon("death");
	for(;;)
...
//Function Number: 24
lib_A7A7::spawnspectator(param_00,param_01)
{
	self notify("spawned");
	self notify("end_respawn");
	self notify("joined_spectators");
	lib_A7A7::in_spawnspectator(param_00,param_01);
}

//Function Number: 25
lib_A7A7::respawn_asspectator(param_00,param_01)
{
	lib_A7A7::in_spawnspectator(param_00,param_01);
}
...
lib_A7A7::in_spawnspectator(param_00,param_01)
{
	...
	lib_A7A7::resetuidvarsonspectate();
	maps\mp\gametypes\_spectating::setspectatepermissions();
	lib_A7A7::onspawnspectator(param_00,param_01);
...
//Function Number: 27
lib_A7A7::getplayerfromclientnum(param_00)
{
	if(param_00 < 0)
	{
		return undefined;
...
//Function Number: 28
lib_A7A7::getrandomspectatorspawnpoint()
{
...
//Function Number: 31
lib_A7A7::spawnendofgame()
{
	if(1)
	{
		maps\mp\_utility::freezecontrolswrapper(1);
		self method_800D();
		lib_A7A7::spawnspectator();
		maps\mp\_utility::freezecontrolswrapper(1);
		self method_800D();
		return;
	}
...
//Function Number: 36
lib_A7A7::callback_playerdisconnect(param_00)
{
	...
	lib_A7A7::removeplayerondisconnect();
...
//Function Number: 38
lib_A7A7::initclientdvarssplitscreenspecific()
{
...
//Function Number: 40
lib_A7A7::getlowestavailableclientid()
{
	var_00 = 0;
...
//Function Number: 41
lib_A7A7::setupsavedactionslots()
{
...
//Function Number: 42
lib_A7A7::logplayerconsoleidandonwifiinmatchdata()
{
...
//Function Number: 44
lib_A7A7::callback_playerconnect()
{
	var_00 = lib_A7A7::getrandomspectatorspawnpoint();
	self setspectatedefaults(var_00.var_2E6,var_00.var_41);
	thread lib_A7A7::notifyconnecting();
	self waittill("begin");
	...
	if(!isdefined(self.connectedpostgame["clientid"]))
	{
		if(game["clientid"] >= 30)
		{
			self.connectedpostgame["clientid"] = lib_A7A7::getlowestavailableclientid();
		}
		else
		{
	...
	self.issniper = 0;
	lib_A7A7::setupsavedactionslots();
	level thread lib_A7A7::monitorplayersegments(self);
	thread maps\mp\_flashgrenades::monitorflash();
	lib_A7A7::resetuidvarsonconnect();
	...
	if(!isdefined(self.connectedpostgame["team"]))
	{
		if(maps\mp\_utility::matchmakinggame() && self.sessionteam != "none")
		{
			...
			{
				thread lib_A7A7::setuioptionsmenu(2);
			}

			thread lib_A7A7::kickifdontspawn();
			return;
		}
		...
		if(!maps\mp\_utility::matchmakinggame() && maps\mp\_utility::allowteamchoice())
		{
			...
			thread lib_A7A7::setuioptionsmenu(1);
			return;
		}
...
//Function Number: 45
lib_A7A7::callback_playermigrated()
{
...
lib_A7A7::kickifdontspawn()
{
	...
	if(self ishost())
	{
		lib_A7A7::kickwait(120);
	}
	else
	{
		lib_A7A7::kickwait(var_00);
	}
...
lib_A7A7::addtoalivecount()
{
	...
	{
		level.afk[var_00]++;
		lib_A7A7::incrementalivecount(var_00);
	}
...
//Function Number: 53
lib_A7A7::incrementalivecount(param_00)
{
	level.alivecount[param_00]++;
}

//Function Number: 54
lib_A7A7::removefromalivecount(param_00)
{
	...
	{
		lib_A7A7::removeallfromlivescount();
	...
	lib_A7A7::decrementalivecount(var_01);
	return maps\mp\gametypes\_gamelogic::updategameevents();
}

//Function Number: 55
lib_A7A7::decrementalivecount(param_00)
{
	level.alivecount[param_00]--;
}
...
//Function Number: 62
lib_A7A7::resetuidvarsondeath()
{
}

//Function Number: 63
lib_A7A7::monitorplayersegments(param_00)
{
	param_00 endon("disconnect");
	level endon("game_ended");
	lib_A7A7::createplayersegmentstats(param_00);
	for(;;)
	{
		param_00 waittill("spawned_player");
		lib_A7A7::recordsegemtdata(param_00);
	}
}
...
//Function Number: 66
lib_A7A7::writesegmentdata(param_00)
{
	...
	var_04 = lib_A7A7::calculatematchplaystyle(var_01,var_02,var_03);
...
//Function Number: 67
lib_A7A7::calculatematchplaystyle(param_00,param_01,param_02)
{
	param_00 = lib_A7A7::normalizeplayersegment(param_00,float(tablelookup("mp/playerSegments.csv",0,"Mean",3)),float(tablelookup("mp/playerSegments.csv",0,"SD",3)));
	param_01 = lib_A7A7::normalizeplayersegment(param_01,float(tablelookup("mp/playerSegments.csv",0,"Mean",2)),float(tablelookup("mp/playerSegments.csv",0,"SD",2)));
	param_02 = lib_A7A7::normalizeplayersegment(param_02,float(tablelookup("mp/playerSegments.csv",0,"Mean",4)),float(tablelookup("mp/playerSegments.csv",0,"SD",4)));
	...
	{
		var_09 = lib_A7A7::getcentroiddistance(var_03,var_08);
		if(var_09 < var_06)
		{
			var_05 = var_08;
			var_06 = var_09;
...
//Function Number: 68
lib_A7A7::normalizeplayersegment(param_00,param_01,param_02)
{
	return param_00 - param_01 / param_02;
}

//Function Number: 69
lib_A7A7::getcentroiddistance(param_00,param_01)
{
...
//Function Number: 70
lib_A7A7::clearpracticeroundlockoutdata(param_00,param_01)
{
...
//Function Number: 71
lib_A7A7::checkpracticeroundlockout(param_00)
{
	...
	if(var_06 > 0)
	{
		lib_A7A7::clearpracticeroundlockoutdata(param_00,var_01);
	}
